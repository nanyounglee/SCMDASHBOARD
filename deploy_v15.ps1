# =============================================================
# SCM Dashboard deploy script v15 (run on PC)
...
# Move any older version snapshots in root to archive (keep only v15)
Get-ChildItem "." -Filter "scm_dashboard_v*.html" -File | Where-Object { $_.Name -ne "scm_dashboard_v15.html" } | ForEach-Object {
  Move-Item $_.FullName "archive\" -Force
  Write-Host "Archived: $($_.Name) -> archive" -ForegroundColor Yellow
}
Copy-Item "index.html" "scm_dashboard_v15.html" -Force
Write-Host "Snapshot created: scm_dashboard_v15.html" -ForegroundColor Cyan

# 1b) Archive superseded v14 docs
New-Item -ItemType Directory -Force -Path "docs\archive" | Out-Null
Get-ChildItem "docs" -Filter "*_v14.html" -File -ErrorAction SilentlyContinue | ForEach-Object {
...
if (Test-Path "deploy_v14.ps1") {
  Remove-Item "deploy_v14.ps1" -Force
  Write-Host "Removed obsolete script: deploy_v14.ps1" -ForegroundColor Yellow
}
...
if ($LASTEXITCODE -ne 0) {
  $msg = "v15 update $(Get-Date -Format 'yyyy-MM-dd HH:mm')"
...
Write-Host "Check: sidebar should show 'SCM.. v15', and try the new period-unit tabs (주별/월별/분기별/기간지정) on 종합현황/발주/매입/이슈/고객인지이슈. 월간 공지사항 카드는 기간지정으로 여러 달을 선택해도 해당 달들의 공지가 모두 표시되는지 확인." -ForegroundColor Green
