...
| **v0.9 (2026.08.04b)** | (로컬→PC 배포) | **사용자 요청 — 매출 실측을 연간분석 표 기준에 정렬(VAT 별도 + 출고 마감 컷오프).** "매출 현황(업로드 실측)"과 표 매출액의 1~7월 누계 격차 +10.7%(6.02억)를 **+0.11%(0.06억)** 로 축소. 원인이 날짜 기준이 아니라 세 가지였음을 실측으로 규명: ① **VAT** — `proj_rev.csv`의 `총 매출액`은 출고리스트 '출고처리 금액'과 같은 VAT 포함 금액인데 매입금액·표는 VAT 별도. `getProjRevMap()`에서 `REV_VAT_DIV`(1.1)로 나누고 원 단위 반올림해 매출 소비처 전체(매출 모달·종합현황 KPI·원가 분석 월별/프로젝트별)에 일괄 적용 — **원가율이 분자만 VAT 제외라 과소계산되던 구조적 오류도 함께 교정**(1~7월 재고제외 35.1%→38.6%, 재고포함 53.0%→58.3%). 근거는 `총 매출액`을 `발주 신청일` 월로 묶고 ÷1.1 하면 표의 **수주액과 원 단위까지 일치**한다는 역검증(1월 -965원·2월 +2,191원·4월 -3,491원·6월 -79,136원; 5·7월 부족분은 수주 후 미출고라 proj_rev에 행이 없는 건). ② **귀속월** — 표는 달력월이 아니라 매달 출고리스트 마감 시점까지의 출고분 합계라 달력월 집계로는 월별 ±30% 편차(3월 -32%·4월 +38%·6월 +27%). 표의 월별 누계에서 역산한 컷오프를 `REV_CUTOFF` 상수로 두고 `최종 출고일` 기준으로 귀속(`revMonthOfShipDate()`), 2026은 01-30/02-24/04-06/04-27/05-26/06-22/07-08. 미확정 월은 달력월 폴백하되 이미 마감된 달로 되돌리지 않고 다음 달로 넘김(07-08 이후 7월 출고분 → 8월분). 종전 `출고 날짜(월)` 컬럼은 `최종 출고일`의 월과 전수 일치(1,433건 중 불일치 0)했으므로 기준일 자체는 바뀌지 않았고, 복수 출고월 균등분할만 최종 출고월 단일 귀속으로 변경. ③ **집계 모수** — `revenueByMonthAll()`/`updateKPIs()`가 발주 RAW 매칭 프로젝트만 집계해 전년 수주·당해 출고 건 **112건 601,563,214원(대부분 1월분)** 이 탈락, 1월 실측이 표의 54% 수준이었음. 표는 출고 시점 기준이라 포함이 맞다는 사용자 확정에 따라 필터 제거(검색어 있을 때만 검색 결과로 한정), 1월 정합도 54%→99.4%. 적용 후 월별 오차 -0.6/+1.0/+2.8/-6.0/+1.4/-8.0/+11.9%(7월은 표가 초안본). 화면에 `cutoffRow()`(마감 출고일)·`diffRow()`(실측−표, 합계는 양쪽 마감월만) 2행 신규 + 카드 비고·모달 안내문 갱신. **운영 주의: 마감일은 매달 달라 고정 규칙이 없으므로 새 달 확정 시 `REV_CUTOFF`에 날짜 추가 필요.** |
