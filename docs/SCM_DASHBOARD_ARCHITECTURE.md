...
### 4-33. 외주생산파트 주간리포트 생성 — genWeeklyOutsource() (v22.16 · v23.8 · 2026.08.05 · 2026.08.09 개편)
...
//   wrParseFinalReport(md): 머리글 `기준주차`와 1-1 표의 "이번 주" 열을 읽는다. 라벨을 정규화해
...
//
// ─ 섹션 5-0: 이슈발생협력사 + 협력사 품절 (v23.8 · 2026.08.09 표 전환) ─
...
// Q(품질): `협력사 | 이번 주 이슈 | 연속 발생 | 진행 상태` — 행은 5-0의
//   issueSupRows를 그대로 재사용(5-0은 실패비용 관점, 7-Q는 조치 관점).
//   진행 상태는 연속 주수에 따라 RCA 검토/집중 모니터링 문구 + "(담당자 작성)".
//   실패비용 집중(YTD 매입 0.3%↑)·품질등급 불일치는 표 아래 인용줄로.
// C(원가): `구분 | 건수 | 내용` — pcDir()로 인상/인하/변동을 나눠 건수와
//   대표 품목(협력사)을 표기. 4-1과 같은 판정 함수를 쓰므로 두 섹션 수치가
//   어긋나지 않음. 복합 리스크(단가변동+이슈 동시)는 표 아래 인용줄로.
// D(납기): `지표 | 값 | 비고` — 미입하(입하예정일 기준, §4-31b)·긴급건수 2행 + "긴급 상위 협력사" 줄.
//   긴급 협력사는 건수 상위 3곳만(2026.08.05: 5곳→3곳, 사용자 요청).
//   긴급률이 전주 대비 3%p 이상 오른 경우에만 검토 제안 인용줄 추가.
```
