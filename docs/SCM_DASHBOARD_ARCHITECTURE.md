...
### 4-33. 외주생산파트 주간리포트 생성 — genWeeklyOutsource() (v22.16 · v23.8 개편)
...
//
// ─ 필드 레지스트리 WR_FIELDS (v23.8 신규) ─────────────────────
// 이 리포트가 참조하는 9개 CSV(order·issue·ci·priceChg·partsMaster·
//   quarterEval·stockout·sup·partsCsv)의 헤더를 함수 상단 한 곳에 정의.
...
//   "PT****-" 접두어 제거.
//
// ─ 섹션 5-1~5-5: 카테고리별 요약 (v23.8 표→줄글) ─────────────
...
// A→"Core", B→"Performer", C→"Developing", D→"General".
//
