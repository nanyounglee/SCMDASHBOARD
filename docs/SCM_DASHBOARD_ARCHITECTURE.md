...
> 버전: v0.9 (2026-09-08) | 갱신일: 2026-09-08 | 작성: 이난영 / Claude AI  
...
// v23.17: delivery_ontime 분자·분모를 판정월(입하예정일+1일) 기준으로 전환 — §4-31b 참조.
...
### 4-31. 미입하율 추이 차트 + 지점 클릭 MoM/YoY (v21.3 · v23.17 기준일 전환)
```javascript
// 미입하율 추이표 카드를 grid-2로 재구성: 좌 차트(#ch-iss-noarrival)+클릭 상세 패널(#iss-noa-info), 우 기존 표.
// renderIssueAnalytics()가 월별 dueCur/noaCur(당해)·duePrev/noaPrev(전년)에서 rate 배열을 만들어
// 라인차트 3계열(당해/전년 점선/목표 1.56% 상수선) 렌더 + ISS_NOA 모듈 상태에 저장.
// v23.17: 분자·분모 모두 판정월(noReceiptMonth = 입하예정일+1일) 기준 — §4-31b 참조.
//   과업지시월 기준 taskCur/taskPrev는 같은 함수 안에서 이슈율 차트 분모로 계속 쓰이므로 분리해 둔다.
...
//   → #iss-noa-info에 해당 월 미입하율·미입하/입하예정 건수·MoM(1월은 전년 12월과 비교)·YoY(%p, 색상 = 상승 빨강) 표시.
// nav issue 진입 시 renderIssueAnalytics() 재호출 — 숨김 상태에서 refreshAll이 그린 이슈 분석 차트
// 3종의 0크기 문제를 CI 차트(§4-30)와 동일 패턴으로 해소.
```

### 4-31b. 미입하 판정 — hasNoReceipt() / noReceiptCheckDate() (v23.17, 2026.08.10)

```javascript
// 미입하를 세는 모든 화면(리포트·차트·KPI 트래커·AI 질의·버퍼 분석·협력사 등급)이 공유하는 단일 판정.
//
// ─ 판정값: order.csv `미입하 발생이력_movement` ────────────────
// Airtable export 포맷이 2026-W31부터 바뀌었다.
//   ~W30: "0 checked out of 1" / "1 checked out of 1"   (체크 수/전체 수)
//   W31~: ""  /  ","  /  "checked"  /  ", checked"      (체크된 movement만 나열)
// 기존 판정이 .includes('1 checked out of') 문자열 매칭이라 신 포맷에서 전 행 매칭 실패 →
//   미입하가 리포트·차트·KPI 전부에서 0건으로 나오고 있었다(2026.08.10 발견, order.csv 6,507행 중 0건).
// hasNoReceipt()는 신·구 포맷을 모두 인식한다. 구 포맷 인식을 지우면 안 되는 이유:
//   CSV_BANK 스냅샷과 전년 아카이브(D.orderPrevYear)가 아직 구 포맷이라 전년 비교 계열이 통째로 죽는다.
// ※ 원천에서 포맷이 또 바뀌면 조용히 0이 된다 — test_miipha.mjs가 order.csv 미입하 0건이면 실패시킨다.
//
// ─ 판정일: noReceiptCheckDate(r) = (입하확정일 || 입하예정일) + 1일 ─
// 물류는 "입하예정일 다음날 09:00에 미입고면 체크"하는 구조. 즉 미입하가 실제로 판정되는 시점은 예정일+1일.
// 외주생산은 TASK를 과업지시일자로 보므로 그대로 버킷하면 리드타임(평균 10일)만큼 뒤로 밀려
//   "이번 주 발주분 중 미입하"는 납기가 매우 짧은 건 외에는 구조적으로 잡히지 않는다(항상 0에 수렴).
// +1일까지 반영하는 이유: 예정일이 집계구간 마지막 날인 건은 체크가 다음날 붙으므로,
//   +1일을 더해야 그 건이 누락(이번 구간 미판정) 없이 다음 구간으로 정확히 넘어간다.
// noReceiptMonth(r)는 판정일의 "YYYY.M" — pOrderMonth()와 같은 키 형식이라 월별 맵에 그대로 꽂힌다.
//
// ─ 적용 범위 (율을 내는 곳만 분모까지 전환) ────────────────────
// 전환함: 주간리포트 1-1·1-2·1-3·7-D / 미입하율 추이 차트(dueCur·duePrev) /
//         KPI 트래커 delivery_ontime(byDueMonth) / AI 질의 "미입하 발생 현황".
//   분모를 "해당 구간에 판정된(입하예정) 건수"로 함께 바꿔야 비율이 성립한다 —
//   분자만 바꾸면 과업지시 건수 대비가 되어 수치가 왜곡된다.
//   과업지시월 모수(taskCur·byMonth.total)는 이슈율·긴급률·리드타임이 계속 쓰므로 건드리지 않고
//   미입하 전용 카운터를 나란히 둔다.
// 전환 안 함: 버퍼 분석 제품별 noRecv, 협력사 등급평가 norecv — 건수만 쓰고,
//   등급평가는 반기 단위라 시차가 희석되며, 분모를 바꾸면 같은 루프에서 세는 발주액·이슈와 모수가 어긋난다.
//
// ─ 남는 한계 ──────────────────────────────────────────────
// 체크가 예정일+1일 09:00에 붙으므로 CSV 동기화가 그보다 이르면 마지막 하루치는 다음 주 리포트에 반영된다.
//   리포트를 목요일 13:00에 뽑고 CSV를 같은 날 오전에 동기화하는 현행 운영에서는 문제되지 않는다.
```

### 4-31c. 단가 이력 구간 변동 조회 — priceTierChange() (v23.18, 2026.08.10)

```javascript
// data/parts_price_history.json = 주간 워크플로(scripts/update_price_history.mjs)가 parts.csv의
//   구간단가(1·50·100·300·…·30000개)를 매번 직전 스냅샷과 비교해 쌓는 에폭 이력.
//   { epochs: { PT1234: [{from:'0000-01-01',tiers:[...]}, {from:'2026-07-29',tiers:[...]}] }, current: {...} }
//
// priceTierChange(ptId, baseDate, tier) → {before,after,pct,when} | {fail:사유}
//   주간리포트 4-1이 "변동 내용 (300개 기준)"을 자동으로 채우는 데 쓴다 — 담당자가 원문에 300개
//   기준을 안 적으면 그동안 단가 히스토리를 수기로 뒤져 넣었다.
//
// ─ 왜 창(window)이 필요한가 ───────────────────────────────────
// 에폭의 from은 update_price_history.mjs가 **워크플로를 실행한 날**이지 실제 단가 변경일이 아니다.
//   그래서 baseDate(요청일자) -1일 ~ +14일 창에서 "첫 에폭"을 이번 변동으로 간주하고,
//   그 직전 에폭을 변경 전 값으로 쓴다. 실측 15건: 대부분 요청일 당일~+1일에 찍힘
//   (cron은 목요일 주 1회지만 "지금 새로고침"으로 자주 실행되어 실질 granularity는 하루 수준).
//   -1일을 두는 이유는 에폭이 요청일보다 하루 먼저 찍히는 경우가 있어서.
//   창 첫 에폭의 index가 0이면(=비교할 직전 에폭 없음) 산출하지 않는다.
//
// ─ 실패를 조용히 넘기지 않는다 ────────────────────────────────
// fail 사유를 그대로 셀에 찍는다: '파츠DB 미반영'(price_chg에만 쓰고 파츠 표준원가는 안 고친 건 —
//   그 자체가 담당자 확인 대상) / 'PT코드 미기재' / 'N개 구간 변동 없음' /
//   '단가이력 미로드'(fetch 전에 리포트를 뽑은 경우 — '미반영'으로 찍으면 전 행이 오탐이 된다).
//
// ─ 커버리지 한계 ─────────────────────────────────────────────
// 이력은 2026-07-21부터 쌓인다. 그 이전 요청건은 복원 불가(실측 134건 중 11건만 산출).
//   추적 개시 이후 요청건은 10건 중 8건(80%) 산출 — 주간 리포트는 최근 1주만 보므로 실질 80%.
// 검증: test_price300.mjs (창 경계·구간 선택·미로드 구분 + 실 price_chg.csv 산출 0건 가드)
```
...
### 4-33. 외주생산파트 주간리포트 생성 — genWeeklyOutsource() (v22.16 · v23.8 · 2026.08.05 · 2026.08.09 · 2026.09.15 개편)

```javascript
// genWeeklyOutsource(): 리포트 생성 화면 "외주생산파트" 카드에서 호출.
// 현재 ISO week 기준으로 7개 섹션의 .md 파일을 생성하고 다운로드.
// (v23.8: 필드 레지스트리 도입 + 카테고리·QCD 표→줄글 + parts.csv 연동)
// (2026.08.05: 1-2 매입금액 기준 분리 + 5/6/7 정비, 아래 각 섹션 참조)
// (2026.08.09: W32 수기 최종본과 대조해 1-1 계층화·2-2 이슈유형·5-0/7 표 전환 등 반영)
// (2026.09.15: 긴급 v2 단독 게재 + W37 수기 최종본 포맷 반영 — 아래 "긴급 판정 v2" 및 각 섹션 참조)
//
// ─ 긴급 판정 v2 — urgV2(rows) (2026-W36 병기 도입 → W37부터 단독, 2026.09.15) ─
// 현행(긴급여부 신청값)은 프로젝트 단위라 task 1건만 긴급이어도 전체가 긴급으로 잡혀
//   주간 변동폭이 과대했다(W35 23.4% → W36 8.7%). 리포트는 task 단위 자동 판정만 쓴다.
//   긴급   = 과업지시일자 + 총 제작 리드타임(영업일, `현 주문수량_아이템 총 제작 리드타임 (from order)`
//            멀티값이면 최대값) ≥ 임가공 예정일(`임가공일자` → 롤업 'M/D (요일)' 폴백, 연도는 과업지시일 기준 추론)
//   고위험 = 임가공 예정일을 3영업일 이상 초과. 미신청 = 자동 판정 긴급인데 긴급여부 ≠ '긴급'.
//   분모(판정 가능 건) = 발주 TASK − 샘플(OS) − 임가공일/리드타임 미등록 − 임가공일이 과업지시일보다 앞선 건.
//   영업일: 주말 + CSV/holidays.csv(date,name). 파일에 당해 연도가 없으면 1-1 아래 ⚠ 한 줄, 집계구간
//   7일 안에 공휴일이 있으면 "영업일 계산에 M/D(이름) 공휴일 반영" 한 줄. 그 외 주석은 안 붙인다.
//   rows에는 긴급 건만 {r, over, lead, oi, pkg}로 담아 7-D 상세표가 그대로 쓴다.
//   isRework(r): task_id에 재제작|추가제작 — 7-D 상세표에서만 제외(1-1 수치는 포함, 제외 건수는 주석).
//   검증: tests/urgent_v2.test.mjs (index.html의 판정 블록을 그대로 평가 — 공휴일·영업일 가산·판정 구조).
//
// ─ 표 셀 정리 헬퍼 cut(v, n=150) (2026.08.09) ─────────────────
// 줄바꿈·연속공백을 한 칸으로 접고 n자로 자른다. 기존엔 각 셀마다
//   String(...).replace(/\n/g,' ').substring(0,40~60) 를 흩어 써서 문장이
//   중간에 끊겼음(W32 최종본에도 잘린 흔적이 남아 있었음). 12곳을 이 헬퍼로
//   통일하고 한도를 120~150자로 상향 — 한도를 바꾸려면 이 기본값만 고치면 됨.
//
// ─ 필드 레지스트리 WR_FIELDS (v23.8 신규, 2026.08.05 quarterEval 제거) ──
// 이 리포트가 참조하는 9개 CSV(order·issue·ci·priceChg·partsMaster·
//   stockout·sup·partsCsv·salesHistory)의 헤더를 함수 상단 한 곳에 정의.
//   pick(r, spec): spec가 배열이면 폴백 우선순위대로 "값 있는 첫 필드" 반환.
//   wrItem(r)=이슈 품목명(입고물품 우선), wrGrades(r)=품질등급 최초/SCM·불일치판정.
//   → 원천 헤더 변경 시 WR_FIELDS 한 곳만 수정. 헤더 오타로 리포트가 조용히
//     빈칸을 뱉던 사고(예: 수량이슈대응방안_SCM)를 구조적으로 차단.
//   (quarterEval 항목은 2026.08.05 섹션 6 개편으로 미사용 — 삭제됨)
//
// ─ 날짜/주차 계산 ─────────────────────────────────────────────
// isoWeekOf(today) → {y, w} / prevIsoWeek(y,w) → 직전주.
// 집계구간: 전주 목 13:00-이번 주 목 12:59. (2026.08.05: 기간 표기 ~ → - 로
//   통일 — 일부 마크다운 렌더러가 "A~B"를 취소선으로 표시하는 문제 회피)
//
// ─ 이슈 날짜/카테고리 집계 ────────────────────────────────────
// issueInWeek: getIssueDateStr → parseAnyDate → "YYYY.M.D" 재조합 후 주차 비교.
// primaryCat(r): 이슈카테고리 첫 토큰만 사용(다중 카테고리 이중집계 방지).
//
// ─ 섹션 1-1: 계층 표 + WoW 표기 (2026.08.09) ─────────────────
// 회의 액션아이템 "합계 대비 구성비 병기" 반영 — TASK/매입금액을 굵은 합계 행으로
//   올리고 발주·재고생산을 ├/└ 하위 행으로 내려 share()로 구성비(%)를 병기.
//   금액 셀 안에 붙어 있던 "(확정 N건)"은 `금액 입력완료율 (발주)/(재고생산)`
//   별도 행으로 분리(fillCell/fillRate, WoW는 %p).
//   ※ v23.29(2026.08.13)에 이 두 행은 삭제됐다 — 공급가액은 발주 직후 거의 비어 있다가 2주 뒤
//     85% 선에서 평탄해져(0~6일 4.7% · 7~13일 61.5% · 14~20일 84.9% · 90일+ 85.5%), 그 주
//     발주분으로 재면 입력 규율이 아니라 경과일을 재는 꼴이고 이번 주 vs 전주는 항상 급락으로
//     나온다. 대신 표 밑에 "매입금액은 공급가액 입력분만 합산 = 확정분 하한" 캡션을 단다.
//     fillCell/fillRate는 그 캡션과 회의록 요약이 계속 쓴다.
// WoW 표기: arrow()로 ▲▼ + 증감률 병기. wow(건수)/wowPct(%p)/wowAmt(금액) 3종.
//   하드코딩 '-'였던 재고생산 매입금액·품질등급 일치율·실패비용 WoW를 실제 계산값으로.
//   ※ 2026.07.30에 매입금액 WoW를 ▲▼→+/-로 바꿨다가 W32 최종본 기준으로 되돌린 것.
//
// ─ 미입하만 다른 기준일 (v23.17, 2026.08.10) ─────────────────
// 1-1·1-2·1-3·7-D의 미입하는 판정일(입하예정일+1일) 기준이고 모수도 "그 구간 입하예정 건수"다
//   (dueInWeek() / noReceiptMonth(), 판정은 공용 hasNoReceipt() — 상세는 §4-31b).
// 표기는 기존 그대로(`미입하 TASK | N건 (P%)`). 기준일·모수 병기를 넣었다가 되돌렸다
//   (2026.08.10 사용자 요청) — 리포트 독자에게는 불필요한 내부 사정이고, 1주 내 1~2건 오차는
//   허용 가능하다는 판단. 기준이 다르다는 사실은 코드 주석과 이 문서에만 남긴다.
//
// ─ 전주 열은 지난주 리포트 게재값 — 주차 스냅샷 (v23.28, 2026.08.13) ─
// 원천 CSV는 사후 입력으로 계속 늘어난다. 실측 — 2026-W32 구간 발주 TASK가 리포트 생성
//   시점 214건(CSV_BANK/2026_W32/order_20260806055123.csv, 6488행) → 1주 뒤 재조회 265건.
//   현 CSV에서 전주를 다시 필터링하면 "이번 주 = 목12:59 컷 / 전주 = 컷 + 1주치 사후 입력"이
//   되어 WoW 감소폭이 과장된다(W33 TASK 합계 -40.1% → 같은 컷 기준 -25.9%).
// order.csv의 `과업_생성일`은 과업지시일자와 같은 값이라 등록 시점 컷오프로 못 쓴다 —
//   현 데이터로 과거 시점을 복원할 수 없어, 생성할 때 그 주 확정값을 저장하는 방식을 택했다.
// wrSnapAll() / wrSnapMerge(label, live) / wrSnapSave(label, snap), 보관은
//   localStorage['wrWeeklySnapshots']. 1-1 전주 열(P 객체)과 1-3 지난 회차가 이 값을 읽고,
//   이번 주 열만 현 데이터로 계산한다.
// 저장 시점은 "생성(다운로드)"이 아니라 **확정 리포트 등록**이다(2026.08.13). 생성 직후 값은
//   회의 전 잠정치라 회의 중 정정이 빠진다 — W32는 등급일치율이 회의 중 75.0%→100.0%로 정정됐다.
//   wrParseFinalReport(md): 머리글 `기준주차`와 1-1 표의 "이번 주" 열을 읽는다. 긴급은 v2 필드
//     (urg2/urgRate2 ← '긴급건수'·'긴급건수(개선안)', hr2/hrRate2 ← '고위험 긴급', unreq2 ← '긴급 미신청')로
//     저장하고 W36 병기본의 '〔참고〕 긴급건수(현행)' 행은 버린다(2026.09.15). 라벨을 정규화해
//   **굵게**·├└·<sup> 꼬리표·정렬 공백을 흡수하고, 1-1 구간만 잘라 훑는다(1-3 추이표에 동명 행이
//   있어 섹션을 안 자르면 덮어써진다). 표에서 뺀 행은 빠진 채로 저장 → 필드 단위 병합이 받아준다.
//   등록 안 된 주차를 전주로 쓰면 1-1 위에 ⚠ 경고 줄이 붙는다.
//
// ─ 미보고 이월 (v23.30, 2026.08.13) ───────────────────────────
// 확정 리포트에 실린 적 없는 이슈를 이번 회차로 끌어온다. 판정 기준은 날짜가 아니라 "실렸는가"다.
//   집계 창이 날짜 단위라 목요일 당일 등록 건은 양쪽에서 빠진다 — 2026.8.6 등록된 고객인지이슈는
//   W32 리포트가 그날 오전에 생성돼 못 담았고 W33 창(8.07~)에도 안 들어가 영구 누락됐다.
// wrParseFinalReport가 섹션 2 표에서 이슈 키를 뽑아 snap.reported={ci,quality,qty,op}에 저장하고,
//   생성기는 "직전 확정 회차의 창 시작 ~ 이번 창 시작 전날"에 발생했는데 그 집합에 없는 건을 고른다.
// 키는 wrKey(품목, 협력사/프로젝트) — **이슈 내용은 넣지 않는다.** 최종본의 "이슈 내용 요약"은
//   담당자가 재작성하는 칸이라 CSV 원문과 글자가 달라, 넣었더니 이월이 26건으로 폭주했다.
// 고객인지이슈만 표에 자동 편입(`<sup>이월</sup>`)한다 — 2-1의 품목·프로젝트는 코드가 원천 값을
//   그대로 찍어 키가 안정적이다(W30~W32 대조 오탐 0). 품질·수량·운영은 품목 셀을 담당자가 다듬어
//   대조가 어긋나므로(품질 1건→8건으로 부풀었다) `2-5. 미보고 후보` 목록으로만 띄우고 수치엔 안 넣는다.
// 훑는 구간을 직전 1회차로 제한 — 전 구간을 보면 오탐이 매주 재등장한다(3주치 15건 → 8건).
// 확정 등록 회차가 없으면 이월도 없다.
//   병합은 필드 단위(Object.assign(live, saved))라 일부 지표만 담긴 시드도 동작하고,
//   스냅샷이 없으면 종전대로 현 데이터에서 재계산한다(값이 틀리는 게 아니라 예전 방식).
// WR_SNAP_SEED: 기능 도입 이전 회차(W30~W33)의 게재값 하드코딩. 같은 주차를 다시 생성하면
//   저장값이 시드를 덮어쓴다. ※ 미입하만 현 판정로직 재산출값 — 당시 게재값 0건은
//   v23.17 이전 로직의 오류값이라 그대로 쓰면 추세가 거꾸로 보인다.
// 자체검증: test_wr_snapshot.mjs (시드 우선순위·부분 시드 병합·미저장 회차·저장 왕복).
//
// ─ 섹션 1-1 vs 1-2: 매입금액 집계 기준 분리 (2026.08.05) ──────
// 1-1(이번 주 KPI)은 과업지시일자 기준(주간 발주 시점 관점) 그대로 유지.
// 1-2(월별 KPI 누적)의 매입금액만 대시보드 전역 helper인 purchaseByMonth()
//   (세금계산서작성월 기준, `합계_분할`로 분할계산서 반영)로 교체 — 기존엔
//   1-2도 과업지시일자로 필터링해 함수 내 안내문(세금계산서 기준은 1-2 참조)과
//   실제 계산이 어긋나 있었음. 두 표 라벨에 각각 기준을 명시.
//
// ─ 섹션 3: 이슈 유형별 (v23.8 중복 정리) ──────────────────────
// 3-1 월별 이슈유형+YTD 합계, 바로 아래 YTD 비중%를 한 줄로. (구 3-2 비중표 삭제)
// 3-2 품질등급 일치율 월별 추이(구 3-3, 품질이슈 건수열은 3-1과 중복이라 제거).
// 3-3 협력사 품질이슈 TOP(구 3-4).
//
// ─ 섹션 2-2: 품질이슈 표 (2026.08.09) ────────────────────────
// 이슈유형: `품질` 고정값 → issue.csv `품질이슈내용구분`(F.issue.qKind).
// 처리결과: 일치면 "B급 (일치)", 불일치일 때만 "B급→C급 (**불일치**)"로 펼침.
// 비용: 이슈표의 `공급가액`이 전 행 공란이라 금액 연동 불가 —
//   `재제작/추가제작`(F.issue.rework) 플래그로 "0원 (정상입고)" /
//   "재제작 (부담주체 확인필요)" 표기. 부담주체 확정은 담당자 수기 영역.
//
// ─ 섹션 4-1: 단가 변동 — 집계 기준·300개 자동산출 (v23.18, 2026.08.10) ─
// 집계: 요청일자가 집계구간(목~목 7일) ±1일 안에 있는 건만. 이전에는 "요청일자 ISO주 ±1주 OR
//   적용시점 ISO주 -1~해당주"라 같은 건이 3주 넘게 계속 남았다(사용자 요청으로 축소).
//   적용시점은 표에 표시만 하고 판정에는 안 쓴다. ISO 주 라벨이 아니라 실제 날짜로 비교한다.
// 300개 기준 자동산출: priceTierChange(pt, 요청일자, 300) — 판정 로직은 PRICE_HISTORY 옆
//   모듈 스코프에 있다(§4-31c). 성공하면 "28,030원 → 18,500원 (-34.0%)", 실패하면 사유 표기.
//   **원문은 항상 병기**한다(사용자 요청 — 담당자 기재 원문이 근거 자료).
//   자동 산출된 건은 pcPct()로 원문에서 %를 또 뽑지 않는다(두 수치가 어긋나 보이는 것 방지).
// pcPt(r): 대상 파츠명 → 굿즈명 → **변동사항 본문** 순으로 PT코드를 찾는다. 대상 필드를 비우고
//   본문에만 "PT5408-..."을 적는 담당자가 있어 협력사 폴백과 단가 조회가 함께 실패하던 것을 수정.
// pcDir(r): 인상/인하 판정 신뢰도 순 — ① 300개 실측 ② 원문 "A원 → B원" 숫자 ③ 키워드.
//   키워드만 보면 뒤집히는 실측 사례: PT5408은 11,000→12,000 인상인데 본문에 "100원 인하된
//   것으로 처리되어"라는 설명이 섞여 7-C가 '단가 인하'로 표기했다. 4-1과 7-C가 같은 함수를 쓴다.
//
// ─ 섹션 4-1: 링크 제거·협력사 폴백 (v23.8 보강, 2026.08.09 증감률) ────────
// 변동 내용에서 마크다운 링크(`[텍스트](url)`, 닫는 괄호 없는 잘린 필드도 매치)를
//   텍스트만 남기고 제거 후 남은 순수 URL도 제거. 협력사가 비면 품목의 PT코드로
//   parts.csv의 제작협력사를 매칭(partsCsvSup). ※ 수주처=제작협력사=거래처=협력사=발주처 동의어.
// 컬럼 헤더는 "변동 내용 (300개 기준)"(회의 액션아이템).
// pcPct(txt): 원문에 "A원 → B원" 패턴이 있고 원문에 %가 없을 때만 증감률을 계산해
//   덧붙인다. 원문 형식이 자유서술이라(변경전/변경후, "N원에서 M원으로" 등) 화살표
//   패턴 외에는 계산하지 않음 — 틀린 %를 붙이느니 원문을 그대로 두는 편이 안전.
//   %는 cut() 이후에 붙여야 잘리지 않음.
//
// ─ 실패비용은 취득원가 기준 (v23.19, 2026.08.10) ──────────────
// 판정은 task_id에 '재제작' 포함(대시보드와 동일), 금액은 **취득원가**(F.order.cost / sumCost()).
//   v23.19 이전에는 매입금액용 sumAmt()(=공급가액)를 실패비용에도 재사용해 대시보드보다 작게 나왔다.
//   재제작 발주는 공급가액을 비워 두고 취득원가·지출결의로만 관리되는 경우가 많다 —
//   실측: 재제작 134건 중 공급가액>0은 18건, 취득원가>0은 20건, 둘 다 0이 114건.
//   대표 사례 PNA50588_Solid커스텀G형박스(L사이즈)_재제작_A1: 공급가액 0 / 취득원가 505,000 /
//   지출결의 있음 — 필터에는 잡히는데 0원을 더하고 있어 "빠진 것처럼" 보였다.
// 적용: 1-1 주간 · 1-2 월별 · 1-3 최근4주 · 4-2 표/필터 · 5-0 협력사별 · 7-Q 실패비용 집중.
//   7-Q 분모(YTD 매입금액)만 공급가액 그대로 — 대시보드 실패비용율과 같은 조합.
// 1-2 월별은 대시보드 공용 reworkCostByMonth()로 배분한다(세금계산서작성월 우선, 없으면 과업지시월).
//   같은 표의 매입금액이 이미 세금계산서월 기준이라 표 내부 정합도 함께 맞는다.
//   1-1/1-3(주 단위)은 세금계산서월로 자를 수 없어 과업지시일자 주간 창 유지 — 총액은 동일.
// ※ 재제작 134건 중 지출결의는 있는데 금액 필드가 빈 건이 49건 — 어느 필드를 쓰든 0원.
//   원천 입력 문제라 코드로 못 채운다.
//
// ─ 섹션 4-2: 재제작률 (v23.8 정리) ────────────────────────────
// failCostF(curOrders)에서 취득원가 0원인 건은 제외(표기 의미 없음).
//   필터 후 남는 행이 없으면 "### 4-2" 헤더째로 섹션 자체를 생략(빈 섹션 미노출).
//
// ─ 섹션 5-1 이슈발생협력사 · 5-2 협력사 품절 (v23.8 · 2026.08.09 표 전환 · 2026.09.15 재배치) ─
// 2026.09.15: 5-0 스냅샷 → «5-1 이슈 발생 협력사», 품절 → «5-2 협력사 품절 현황 (YYYY.M)»,
//   카테고리 요약 5-1→5-3, 재배치 검토표 5-2→5-4. 품절 소스는 D.sales_status_history 하나 —
//   신규·유지 표 = raw end time 공란 & 판매상태 '일시품절'(파츠별 최신 구간 1행, 변경일 내림차순),
//   해제 표 = 집계구간에 end time이 찍혔고 그 파츠에 열린 품절 구간(품절예정 포함)이 없는 것.
//   컬럼 `품목명(PT코드 포함) | 수주처 | 재고소유(재고소유구분 (from sync_parts)) | 비고(변경사유 원문)`.
//   MANUAL_STOCKOUT(2026.7 고정)·stockout_list.csv(2026.6.19에서 멈춤)는 리포트에서 참조 제거 —
//   W37 최종본 10건이 history 열린 일시품절 행과 정확히 일치했다.
// 이슈발생협력사: repeatSups(2주 연속) 협력사를 표로 출력 —
//   `협력사 | 이번 주 이슈 | 연속 발생 | 실패비용`. 1주차(신규) 발생분은
//   "신규" 표현이 모호하다는 피드백으로 목록에서 완전히 제외.
//   issueSupRows를 여기서 한 번만 계산해 7-Q에서 재사용(같은 협력사 집합을
//   두 섹션이 따로 세지 않게 함).
//   crit(중요도 중~상 heuristic): 품질등급 C·D 또는 재제작(실패비용) 발생.
//   ※ 2026.08.09 버그 수정 — 기존 판정이 new Set(['C','D']) 완전일치라
//     실제 값 'C급'과 한 번도 매칭되지 않았음(표가 항상 "없음"). 접두 1글자 비교로 교정.
//   issueSummaryOf(sup): 이번 주 이슈를 유형별로 묶고 품질은 등급 내역까지
//     펼침(예: "품질 3건 (C급 2·B급 1) · 운영 1건").
//   consecOf(sup): weekSupSets(이번 주부터 과거 최대 12주)를 앞에서부터 훑어
//     끊기는 지점까지의 주 수와 시작 주차를 반환. 3주 이상이면
//     "2026-W26 이후 매주 발생 (7주 연속)", 아니면 "2주 연속". 12주에서 잘리면 '+' 표기.
//     7-Q의 3주 연속 판정도 이 함수를 재사용(주차 세는 로직 이원화 방지).
// 품절 목록 전주 대비 변동(stockoutDelta, 2026.08.09): D.sales_status_history에서
//   판매상태에 '품절'이 포함된 행 중 변경일이 이번 집계구간이면 신규,
//   end time이 이번 구간이면 해제로 집계. 둘 다 없으면
//   "전주(W##) 대비 신규 추가·해제 없음. 목록 동일."
...
//   최종폴백. 반환하는 카테고리는 화면엔 안 쓰지만 5-1 그루핑에서 별도로
//   다시 조회(연결은 유지, 노출만 안 함). 카테고리가 "Drinkware, 기타"처럼
//   다중값이면 '기타'만 걸러내고 나머지를 join. 표시 품명은 stripPt()로
//   "PT****-" 접두어 제거.
//
// ─ 섹션 5-1: 카테고리별 요약 (v23.8 표→줄글, 2026.08.09 건별 한 줄) ─
// parts_master 굿즈카테고리로 이슈·단가·품절을 그루핑. '기타' 카테고리 제외.
// 2026.08.09: 유형별로 제품명을 묶던 방식("A, B 운영 이슈") → 건별 한 줄로 전환 —
//   "제품명(협력사) 운영이슈 — 대응 요약" / "제품명 단가 인상(+9.1%) — 원인" /
//   "제품명 일시품절 — 사유". respOfIssue(r)가 이슈 유형에 맞는 SCM 대응 필드
//   (품질=qResp/수량=qtyResp/운영=opResp)를 골라 60자로 줄인다.
//   품절 사유는 currentReasonOf() 재사용(조회는 PT코드 포함 원본 파츠명으로,
//   표시만 stripPt — 스트립한 이름으로 조회하면 이력이 안 잡힘).
//   dedupeCommas(): 굿즈연결이 "A, A, B"처럼 중복 나열로 들어오는 경우 정리.
// 맨 뒤에 [리뉴얼·의사결정 대기] 입력 슬롯 고정 출력 — 회의 논의분(리뉴얼 진행·
//   S&OP 의사결정 대기)은 CSV에 없어 자동 집계 불가, 담당자 수기 영역.
//
// ─ 섹션 5-2: 재배치 검토표 (2026.08.05: 5-6→5-2 번호 정정) ───
// VC_CHANGE_CACHE(scm_vendor_change bank)에서 확정/완료일이 이번 주인 건 표시.
// 섹션 5가 실제로는 5-0(스냅샷)·5-1(카테고리 요약)·5-2(재배치 검토표) 3개뿐이라
//   기존 5-1~5-5 / 5-6 표기가 실제 구조와 맞지 않았던 것을 정리.
//
// ─ 섹션 6: 공급망 포트폴리오 Tier 분포 (2026.08.05 개편) ─────
// 기존엔 리포트 안에서 D.quarter_eval의 1Q/2Q_공급망관리등급 컬럼을 별도
//   재계산했는데, 이는 공급망 포트폴리오 탭·"A등급 비율" KPI 카드가 쓰는
//   정식 기준(getEvaluatedPortfolioVendors, §4-11 포트폴리오 참조)과 달라
//   화면 간 수치가 어긋났음. 그 함수를 그대로 재사용하도록 교체.
// 처음엔 등급(v._grade, A/B/C/D)으로 표시했으나, 사용자가 대시보드 포트폴리오
//   탭의 Tier 카드(Core/Performer/Developing/General)를 캡처해 "이 기준으로
//   바꿔야 한다"고 확인 — v._quad(QCD×관계 4분면 Tier) 기준으로 최종 교체.
//   실제 CSV 재실행 결과 6/5/22/113개사(4.1%/3.4%/15.1%/77.4%)가 포트폴리오
//   탭 캡처 수치와 정확히 일치함을 확인. Tier와 등급(A/B/C/D)은 QCD/관계
//   별도 축이라 1:1 대응하지 않음(원본 §4-11 포트폴리오 설명과 동일 주의사항).
//
// ─ 섹션 7: 운영 전략 제안 QCD (v23.8 표→줄글, 2026.08.09 다시 표) ─
// W32 최종본이 표 기반이라 줄글에서 표로 되돌림. 이모지(🔴🟠🟡)는 제거.
// Q(품질, 2026.09.15 표→bullet): 5-1의 issueSupRows를 재사용해 3주 연속 이상은
//   "**A(4주 연속)·B(3주 연속)** — 근본원인 분석·개선계획 수령 검토 필요 (5-1 참조)",
//   2주 연속은 "— 집중 모니터링 (5-1 참조)" 한 줄씩. 실패비용 집중(YTD 매입 0.3%↑)·
//   품질등급 불일치는 그대로 인용줄.
// C(원가): `구분 | 건수 | 내용` — pcDir()로 인상/인하/변동을 나눠 건수와
//   대표 품목(협력사)을 표기. 4-1과 같은 판정 함수를 쓰므로 두 섹션 수치가
//   어긋나지 않음. 복합 리스크(단가변동+이슈 동시)는 표 아래 인용줄로.
// D(납기, 2026.09.15 W37 포맷): "> 미입하 N건 · 긴급 N건 · 고위험 N건 — 수치는 1-1 참조" 한 줄 →
//   **미입하** 협력사(건수) 줄 → **긴급 상위 협력사** 상위 4곳 → **고위험 긴급 N건** 상세표
//   `협력사 | task_id | 리드타임 | 과업지시일 | 임가공 예정일 | 초과` → **긴급 미신청 N건** 상세표
//   `협력사 | task_id | 임가공 일정 초과`(초과 내림차순). 두 표 모두 isRework 건은 빼고 "> 재제작, 추가제작
//   건 제외 (n건)" 주석. 긴급률이 전주(스냅샷 urgRate2) 대비 3%p 이상 오르면 검토 제안 인용줄.
```

### 4-33b. 외주생산파트 회의록 템플릿 — genMeetingTemplate() (2026.08.09 개편)

```javascript
// genMeetingTemplate(): 리포트 생성 화면에서 호출, 회의록 초안 .md 다운로드.
// 2026.08.09: YAML frontmatter → 주간보고와 같은 머리글로 교체
//   (# [주간 외주생산 회의록] + 기준일·기준주차(ISO 주 범위) + 작성/참여).
//
// ─ §1 KPI 리뷰: 리포트 결과 재사용 ───────────────────────────
// genWeeklyOutsource()가 실행 끝에 모듈 변수 WR_KPI_SUMMARY = {label, lines[7]}
//   에 핵심 지표를 남기고, genMeetingTemplate()이 label이 이번 주차와 같을 때만
//   그대로 출력한다. 회의록은 항상 리포트를 뽑은 뒤 생성하므로 집계 로직을
//   두 벌 두지 않기 위한 장치 — 리포트를 먼저 안 뽑았으면 빈칸 템플릿으로 대체.
//
// ─ §6 Action Items ───────────────────────────────────────────
// 담당자별 표 4개(외주생산파트·이난영·김민지·육승미), 컬럼 구분·내용·상태.
//   원본은 SCM실 회의 Airtable "회의록 리뷰_생산관리파트"이고 표는 회의 시점
//   스냅샷이라는 안내 주석을 하단에 고정 출력.
// genMeetingDraftFromAirtable()이 클립보드에 넣는 프롬프트의 형식 지시문도
//   같은 구조(담당자별 표 4개)로 맞춰 둠 — 한쪽만 바꾸면 초안 형식이 어긋남.
```

### 4-34. KPI 재고 지표 재정의 + 미구현 지표 명시 (v23.38, 2026.09.07)

```javascript
// 문제: 재고 KPI 2건이 실적과 무관하게 영구 미달로 찍히고 있었다 — 분자와 목표의 축이 달랐다.
//   ① inv_turnover : turnover_3m(3개월, 실측 0.45~1.07)을 연간 목표 6회와 비교
//   ② inv_stockout : SKU 기준 실측(stockout_count/managed_count)을 굿즈 기준 목표 3%와 비교
//
// 해소: 목표축을 지표별로 분리한다. 목표값을 바꾸는 게 아니라 "비교 대상"을 맞추는 일이다.
inv_turnover      3M 회전율        목표 1.0   ← TFT 지표정의서 "3개월 회전율 1"
inv_turnover_yr   연환산 회전율     목표 6.0   ← turnover_ytd × 365 / 연초경과일수 (사전집계에 이미 있음)
inv_stockout      비의도적 품절률(SKU) 목표 2.5% ← 굿즈 기준 3%와는 별개 축

// 신규 2종 — 신규 원천 수집 없이 기존 CSV만으로 산출된다.
inv_stagnant      장기미회전 재고 비율  psrStagnantList_() 재사용(졸업 검토 탭과 동일 판정)
inv_stockout_days 품절 해소 소요일     sales_status_history의 닫힌 상태 구간 중앙값

// ⚠️ inv_stagnant의 분모: period_summary.inv_amount(전체 재고)를 쓰면 분자(관리대상
//   Product Parts 한정)와 모집단이 어긋나 비율이 구조적으로 낮게 나온다. 분모도
//   getAggSkuRows()에 isAggManaged·isAggProduct를 걸어 같은 모집단에서 재합산한다.
// ⚠️ inv_stockout_days: 평균은 최대 586일짜리 장기 건에 끌려가므로 중앙값을 쓴다.
//   'end time'이 빈 행은 미해소 건이라 제외 — 'end time(인터페이스용)'은 미해소 건도
//   채워져 있어(2,252/2,252) 그대로 쓰면 소요일이 부풀려진다. '의도적 품절'은 집계 제외.

// KPI_PENDING — 직무상 필요하지만 원천이 없어 산출 불가한 3종을 화면에 남긴다.
//   폐기율          → 폐기 실적 테이블 필요(폐기증빙필요여부 265건은 트리거일 뿐 실적 아님)
//   예측정확도       → 예측치 주차 스냅샷 필요(dashboard_sku_forecast.csv)
//   재고 Aging 버킷  → 2026-01-01 기초재고 레이어 1회분만 신규. 입고 이벤트(실제입하일 93.6%·
//                     입고수량 93.0%·산출물_파츠코드 88.4%)와 소진(월 판매량 FIFO)은 이미 있고,
//                     아카이브(2024·2025)에 입고수량·파츠코드 컬럼이 없어 과거만 재구성 불가.
// 구현되어 KPI_DEFS로 올라가면 KPI_PENDING에서 지워야 한다(중복은 회귀 테스트가 검출).

// 회귀 테스트: node test_kpi_inventory.mjs
//   실제 computeKPIs()를 실제 CSV에 물려 ①목표축 ②분모 모집단 ③미해소 건 혼입
//   ④도움말 지표 수와 KPI_DEFS.length 일치 ⑤KPI_PENDING/KPI_DEFS 중복을 검사한다.
```
...
│       ├── 캘린더 삼각대 재고[v1.0 · 2026-09-15] — 09-14 기준 재고 − 이후 발주의 발주지시수량(재고생산 제외) · tripodStock()·renderTripodStock()
...
| 버전 | 커밋 | 내용 |
|---|---|---|
| **v1.0 · 2026-09-15 (마진률)** | (배포 커밋) | 매출 현황 모달 후속(사용자 지시). ① 표기 «이익률» → «마진률»(원가율 카드 비고·표 `ratioRow` 행·누적 행·안내문), 금액 «이익» → «마진». 변수 `profitRate`→`marginRate`, `pCol`→`mCol`. ② 목표 `MARGIN_RATE_TARGET=35`(목표 평균 마진률, 사용자 확정) — `mCol`: 35 미만 빨강 · 35~40 주황 · 40 이상 초록(원가율 `cCol`의 5%p 근접 구간과 같은 방식). 직전의 `100−COST_RATE_TARGET`(55%) 파생 목표는 폐기. ③ 누적 현황 KPI 행(카드 4장)과 전용 변수(`cumN`·`cumI`·`cumTarget`·`cumAct`·`cumCost`·`cumAch`·`cumCostRate`·`faS`) 제거 — 누적은 표의 `cumRow` 6행에서만. ⚠ `COST_RATE_TARGET`(45)은 유지되어 원가율 45~65% 구간에서 원가율 행(초과)과 마진률 행(달성)의 판정이 엇갈린다 — 두 목표 정합은 사용자 결정 대기. |
| **v1.0 · 2026-09-15 (매출 누적·이익률)** | (배포 커밋) | **매출 현황 모달(`renderRevenueModal`)에 누적 현황 + 매입원가 대비 이익률**(사용자 요청). 이익률 = (`revenueByMonthAll` − `projectCostByMonth`) ÷ `revenueByMonthAll` = 100 − 원가율(프로젝트 귀속). 색은 `pCol = cCol(100−v)`, 목표 `100−COST_RATE_TARGET`(55%) 이상. ① 원가율 카드 비고에 선택 기간 이익률 ② 2번째 KPI 행 «누적 현황»: 1월 ~ `max(planSelMonthIdx())` 합계(`cumTarget`·`cumAct`·`cumCost`)로 누적 목표 수주액 / 누적 매출·목표 대비 / 누적 매입원가·원가율 / 누적 이익률·이익(음수는 `faS`로 부호 표기) ③ 표: `ratioRow` 이익률 행 + 구분 행 + `cumRow` 누적 6행(목표 수주액·수주액·목표 대비(수주)·매출·목표 대비(매출)·이익률, 합계 열 «—»). 누적 행의 수주·매출·원가는 `nowIdx`(오늘이 속한 달 · 대상 연도가 지났으면 11 · 아직이면 −1)까지만 채운다 — 이후 달은 목표만 누적돼 달성률이 떨어져 보이기 때문(10·11월분으로 이미 귀속된 매출이 있음). 누적 목표 수주액은 12월까지. 실측(2026-W38 데이터, 9월 선택): 1~9월 누적 매출 75.9억·원가 38.8억·이익률 48.9%, 연간 이익률 49%. |
| **v1.0 · 2026-09-15 (삼각대 재고)** | (배포 커밋) | **종합 현황 «캘린더 삼각대 재고» 카드**(사용자 지시 2026-09-14). 상수 `TRIPOD_BASE_DATE`('2026-09-14')·`TRIPOD_POOLS`(이름·기준 수량·PT 목록 — 공용 재고는 PT 여러 개: 그레이 PT5571·PT2804 / 블랙 PT5572·PT1324·PT2803). `tripodStock(rows)`: 과업지시일자 ≥ 기준일이고 `!isStock(r)`인 행에서 `orderRowParts(r)`(sync_itemdb PT 정확 일치)가 풀 PT와 겹치면 `pa(발주지시수량)`을 차감(풀당 행 1회, 사용자 확정 — 여분 포함 수량). `renderTripodStock()`은 `allOrderRows()`(당해+아카이브 — 연말 이관 후에도 누계 유지)로 `#tripod-stock-body`를 그리며 `refreshAll()`에서 호출된다(아카이브 로드 후 재호출). 검색·기간 필터와 무관. 재고생산 제외 이유: 2024 삼각대 재고생산 12행이 같은 PT. 한 행에 두 재고가 섞인 발주는 2024~2026 실측 0건. 입고 가산은 미구현 — 입고 시 `TRIPOD_POOLS[].base` 갱신. 박스 `title`에 차감 내역. 테스트 `test_tripod_stock.mjs`. |
| **v1.0 · 2026-09-14 (문서)** | (배포 커밋) | 사용자가이드·데이터 로직 설명서 v1.0 전면 개정. 원고 `docs/src/SCM_DASHBOARD_사용자가이드.html`·`docs/src/SCM_DASHBOARD_로직설명.html`을 저장소에 두고 `scripts/build_docs_pdf.mjs`(puppeteer + 설치된 Chrome, A4·쪽번호 footer)로 `docs/SCM_DASHBOARD_*_v1.0.pdf` 생성 — 7월판은 원고 없이 PDF만 커밋돼 갱신 불가였다. 7월판 v23 PDF는 `docs/archive/`로 이동. `DC_DOCS`(자료실) 경로·기준일 갱신. 매입금액 해석 안내 추가: 대시보드 매입 = 발주 행 공급가액의 세금계산서월 귀속이라 지출결의 원장과 다름(2024 실측 원장 8,293,795,698 − 대시보드 연간 7,356,159,898 = 발주 미연결 지출결의 438,193,555 + 연결 지출결의 대비 발주 행 금액 부족 504,686,843 − 역방향 5,244,599). 발주 진행현황 카드 `card-sub`와 도움말(`progress`) 문구의 수집 시각을 «매주 월요일 09:00» → «매주 목요일 13:00»으로 수정 — v20 cron 변경(`0 4 * * 4`) 뒤 화면 문구만 남아 있었다. |
| **v1.0 · 2026-09-14 (매출 목표 표)** | (배포 커밋) | `SALES_PLAN`을 2026-09-14 연간분석 01-A/01-B 표로 교체 — `target` 11·12월, `curOrder` 3월(08-03 수정분 원복)·8월·9월, `curRevenue` 6월(730,000,000→1,129,540,000)·7월(588,846,391→776,560,000), `prevOrder` 11·12월. 표의 월 칸은 만원 반올림이라 배열 합계가 표 합계와 1만원 차이 날 수 있다. 새 표 기준 `renderRevenueModal` «실측 − 표 (매출)»: 1~5월 ±3.2% 이내 유지, 6월 -34.3%·7월 -25.8%. `REV_CUTOFF` 6·7월분(07-08·08-04)은 08-03 표에 맞춘 값이라 표 집계 기준 변경 여부 확인 전까지 조정 보류(주석에 명시). |
| **v1.0 · 2026-09-14 (추가)** | (배포 커밋) | **견적 계산기 외주 2026-09-14판 반영 — 최소마진 20만 고정 복귀.** 09-11판 대비 산출 로직 차이는 최소마진 한 곳뿐(박스·쇼핑백 두 분기 모두 수량 구간제 → 20만 고정)이고, 나머지는 PET 선택 시 원단 칸의 장당단가·안내문을 숨긴 화면 변경이다(대시보드엔 해당 패널 없음 → 미반영). `qcCalc`: 수량 구간 `minMargin` 제거 → `Math.max(totalCost×마진율, 200000)`. 입력 안내문·페이지 설명·결과 카드 기준 표기를 09-14판으로 갱신. `build_quote_xlsx.py`: 마진 최소금액 3구간 상수와 «적용 최소마진» 행을 걷어내고 단일 상수(20만)로 복귀. `test_quote_v10.mjs` §4를 옛 구간 경계 수량에서 20만 고정으로 갱신. |
| **v1.0 · 2026-09-14** | (배포 커밋) | **정식 배포 — 표기 v0.9 → v1.0**(`<title>`·`.sb-logo-sub`, 2027년까지 이 체제). ① **견적 계산기 외주 2026-09-11판**: `QC_PAPER_PRICES` 27 → 40지종(원본 키 대소문자 준수 · 구 이력의 소문자 키는 `qcPaperKey()`가 복원), PET 동적단가(`QC_PET_THICKNESS`·`QC_PET_KG`, `qcFindPaper(type,size,pw,ph)` — PET는 `size:'-'`, 연수 판정은 추천 표준지로), UV(`QC_UV_DEGREE`·`QC_UV_BASE`·`QC_UV_PER`, part0 1회), 관리비 최소 12만, 최소마진 수량 구간제(`qcCalc` 반환에 `minMargin`). 뉴에코블랙400g은 원본 13,500(350g의 12배 — 자릿수 오타) 대신 1,350 채택(사용자 확정, `test_quote_v10.mjs`가 고정). 외주판 쇼핑백 분기는 미정의 변수 참조로 원본에서도 죽은 코드 — 견적 계산기 대상에서 제외 확정. ② **엑셀 견적서**: `scripts/gen_quote_constants.mjs` 신설 — `quote_constants.json`을 index.html 엔진에서 생성(수작업 스냅샷 폐지). `qc_engine_export.mjs`: `grabConst` 멀티라인 리터럴 대응, 신규 상수 추출, 파생 지류가 정식 지종을 덮으면 throw. `build_quote_xlsx.py`: 최소마진 3구간 상수 + `적용 최소마진` 수식. ③ `CSV/proj_rev.csv`를 2026-09-14 사용자 제공 export로 교체(1,885건 · 11컬럼 — `getProjRevMap`·`getRevenueByPNA`가 읽는 4컬럼 전부 포함. W37 자동 수집본 1,876건·390컬럼·23.7MB의 상위집합 → 0.4MB). 메인 «매출 현황» KPI는 `projSeen`(2026 발주 RAW 비재고 프로젝트) 필터를 거치므로 결산 행이 늘어도 `order.csv`에 없는 프로젝트는 집계되지 않는다. 교체 전후 실측(order.csv W37): 9월 965,373,309 → 965,425,809원, 7·8·10월 불변. ④ `renderYearlyYoY`: `YOY_MIN_YEAR=2024`로 2022~2023 행 제외(2024 YoY는 자동 «—»). ⑤ 상단 📥 자료실 `openDownloadCenter()`: 문서 4종 + 참조 데이터 전량. 데이터 목록은 스크립트 첫머리의 `fetch` 래퍼가 기록한 `DC_LOADED`(성공한 동일 출처 GET 중 `CSV/`·`CSV_BANK/`·`data/` — 폴백 경로·이력 아카이브·월별/주간 스냅샷까지 런타임 실측, 손으로 관리하는 목록 없음)를 서버 파일 직링크(`<a download>`, 메모리 재적재 없음)로 내려주고, `UD[k]!=='자동로드'`인 수동 업로드분만 `D[k]`를 `dcCsvText`로 직렬화(셀 내 줄바꿈 보존). 업로드로 대체된 배포 파일은 출처 칸에 표시(`window._autoCsvMap`으로 파일→key 역매핑). ⑥ **후속 확정(같은 날)**: UV 별색 환산(별색 1도 = 2도)은 의도한 값. 매출 귀속 마감을 8월분부터 **월 말일 고정**으로 확정 — `REV_MONTHEND_FROM=202608`, `revCutoffLabel()`이 `REV_CUTOFF` 배열 밖의 달(8월분~)에 월 말일을 반환(로컬 날짜로 조립 — `toISOString()`은 KST에서 하루 밀림). `revMonthOfShipDate()`의 달력월 폴백이 이미 월 말일 마감과 같아 집계 금액은 불변이고, 매출 현황 카드·매출 목표 표에만 마감일이 표시된다. 테스트: `test_quote_v10.mjs`·`test_rev_cutoff.mjs`(2026-08-05~2028-12-31 전 날짜 경계 불변식) 신설. |
...
| **v0.9 (2026.09.08c)** | (로컬→PC 배포) | **사용자 제보 — 견적 계산기가 쓰기 어렵다.** 입력부의 작은 드롭다운·체크박스를 누르는 버튼(칩)으로 교체하고, [견적 산출] 버튼을 없애 조건이 바뀌는 즉시 재계산한다(`qcLive()`). 결과 카드 최상단에 개당 소비자가를 큰 글씨로 고정 표시하는 요약 바 신규(`.qc-price`, sticky). 제작수량 프리셋 6종·마진율 프리셋 6종 버튼 추가, 입력을 5단계(기본·원단·인쇄후가공·골판지·추가비용마진)로 묶음. 지종 27종·규격 15종은 선택지가 많아 드롭다운 유지. **계산 로직 무변경**: 네이티브 `<select>`/`<input type=checkbox>`를 `display:none`으로 남겨 두고 버튼이 그 값만 바꾼 뒤 `change`를 디스패치하는 구조라 `qcState()`·`qcCalc()`·`qcLoadHist()`·`qcToggleSections()`가 그대로 동작한다 — `qcSyncChips()`가 숨긴 값에서 버튼 표시를 되맞춘다(이력 불러오기 후 필수). 인쇄 기본값을 켜짐으로 변경(수식 엑셀 기본값과 일치). `qcSave()`가 `QC_LAST.inp`(마지막 계산 시점)에서 견적서 이름을 읽어 계산 뒤 이름만 고치면 옛 이름이 저장되던 버그 수정 — 저장 시점 입력칸에서 직접 읽는다. 검증은 브라우저에서 10단계를 클릭하며 매 단계 화면 표시값과 `qcCalc()` 직접 계산값을 대조(10/10 일치). |
| **v0.9 (2026.09.08b)** | (로컬→PC 배포) | **사용자 요청 — 견적서 이름 이력 저장 + 수식 엑셀 견적서.** ① 견적 계산기 「견적 조건 입력」 최상단에 `qc-name` 자유 입력칸 신규 — `qcState()`가 `name`으로 담고 `qcSave()`가 이력에 함께 저장, 이력 표 첫 열에 「견적서 이름」 추가(빈 값은 `(이름 없음)`), `qcLoadHist()`가 이름까지 복원, `qcExportCSV()` 첫 열로 포함. CSV는 이름에 콤마·따옴표가 들어가도 열이 밀리지 않게 RFC 4180 인용 처리(나머지 칸은 숫자·고정값이라 종전대로). ② **수식 엑셀 견적서 신규** — `scripts/build_quote_xlsx.py`가 시트 2장(`견적계산기`·`단가표`)짜리 `.xlsx`를 생성한다. **매크로(VBA) 미사용**: 계산이 전부 사칙연산·구간표 조회라 수식으로 충분하고, Office가 인터넷에서 받은 `.xlsm`의 VBA를 Mark of the Web 기준으로 기본 차단하기 때문(수식 파일은 Google Sheets·Numbers에서도 동작). 전개사이즈·규격 자동상향(우측→좌측 재귀 IF 격자)·연수·원가 명세 24행·요약 10항목이 전부 수식이고, 최소금액·계수까지 전부 「단가표」 시트 셀 참조라 수식 안에 하드코딩된 숫자가 없다. 단가표는 `index.html`에서 추출한 `scripts/quote_constants.json`을 쓴다(복붙본을 두면 대시보드와 갈라짐). 검증은 `test_quote_xlsx.py` — **실제 Excel(COM)**로 열어 입력을 써 넣고 재계산시킨 뒤 `index.html`의 `qcCalc()` 원문 결과와 대조한다(9 시나리오 × 8 지표 + 전 수식 셀 오류 검사 = 605건, 전부 일치·오류 0). 엑셀 산출물은 단가표가 통째로 들어 있어 `.gitignore`로 저장소에서 제외 — GitHub Pages에 올라가면 링크만 알면 누구나 받을 수 있고, 배포 스크립트의 `git add -A`에 딸려 들어가는 것도 막았다. |
| **v0.9 (2026.09.08)** | (로컬→PC 배포) | **사용자 제보 — 굿즈 상세 모달 발주수량이 실제 주문의 2배.** 리멤버 타투스티커(TTSK) 26.8월 100개 주문이 210개(본품 100 + 배경지 110), 26.9월 1,200개 주문이 2,450개(1,200 + 1,250)로 잡혀 제품별 판매량 추이(8월 100개)와 어긋남. 원인은 **한 고객 주문의 파츠 행을 그대로 합산**한 것 — 한 굿즈 주문은 본품(Product)·부자재(Package)·인쇄(Printing)가 각각 별도 발주 행이고, 종전 코드(`hasAsm`)는 완제품(Phantom) 행이 있는 굿즈만 그 행으로 좁혀서 ① 완제품 행이 아예 없는 굿즈(타투스티커·칫솔살균기 등 525 task)는 전체 행이 더해지고 ② 완제품 행이 2개 이상인 세트(단우산 우산+펠트봉투, 골프공+싸바리박스)도 이중계산됐다. `orderUnitRows()` 신규 — **«고객이 주문한 프로젝트» 단위**로 접는다: `task_id`에서 파츠 접미(`_A1`·`_B1`·`_C1`…)를 뗀 값이 곧 «프로젝트 × 굿즈 1건»(실측 3,893단위, 재제작은 `task_id`에 `재제작`이 들어가 자연히 다른 단위로 갈림)이고, 그 안에서 **최상위 역할(완제품 > 제품 파츠 > 나머지)의 최대 수량 1건만 대표**로 쓴다(합산 금지). 완제품 행 수량이 `0`인 원본이 있어 그때만 전체 행에서 재선정. 적용 범위는 `openGoodsDetail()`에서 수량을 쓰는 4곳 전부 — 요약 칩·연도별 발주 이력 표·옵션별 발주 비중 표/도넛·연도별 월간 발주 추이 표/차트(TTSK 2026년 8,000 → 3,925개, 제품별 판매량 추이와 일치). **공급가액은 부자재 포함 전체 행 합계 유지** — `yearBreakdownHtml()`에 `qtyRows` 인자를 추가해 수량·TASK만 주문 단위로 세고 금액은 전체 행에서 더한다(제품 상세 모달은 산출물 1종만 봐서 종전 동작 그대로). 9월 발주 자체는 실재(PNA53734-차병원서울역센터 · 2026.9.3 · 1,200개 · `PO076394`) — 추이 화면에서 9월이 비어 보이는 것은 1,200개가 빅딜 컷(1,094개, μ+1.28σ)을 넘어 「빅딜 제외」로 빠진 것이며 2026-08-04 확정 규칙이라 유지. 회귀 테스트 `test_order_unit.mjs` 신규(`index.html`에서 함수 원문을 뽑아 실제 `CSV/order.csv`로 검증). **같은 배포 — 견적 계산기 박스 형태 명칭 변경**(사용자 요청): 마닐라박스→**단박스**, C형→**테일러드 스트랩 박스 커스텀**, W형→**SolidG형 커스텀**, 슬리브형→**open페이퍼박스/motion박스**, 후가공 체크박스는 `접착(단박스)`. 전개사이즈 산식·골판지 적용 규칙·단가표는 불변(100×80×60/1,000개 개당원가 458·722·458·722원으로 변경 전과 동일). `localStorage` 견적 이력을 복원할 때 select에 없는 옛 값이 조용히 기본값으로 바뀌는 것을 막으려 `QC_TYPE_ALIAS` 매핑 추가. |
| **v0.9 (2026.09.07)** | (로컬→PC 배포) | **진단 실행계획 ①(2026-08-25 인수인계 문서 §4 Wave1) — 재고 KPI 목표축 정합.** ① `KPI_DEFS`의 `inv_turnover`가 3개월 회전율(`turnover_3m`, 실측 0.45~1.07)을 **연간 목표 6회**와 비교해 정의상 영구 미달이던 것을 3M 목표 1.0(TFT 지표정의서 기준)으로 정정하고, 연간 목표 6회는 `inv_turnover_yr`(**재고회전율(연환산)**) 신설 지표로 분리 — 값은 사전집계에 이미 있는 `turnover_ytd × 365 ÷ 연초경과일수`(`computeKPIs()` 재고 블록)라 S&OP 시트 신규 필드 요청 없이 산출. ② `inv_stockout`은 굿즈 기준 목표 3%와 SKU 기준 실측(`stockout_count/managed_count`)이 섞여 있어 지표명을 `비의도적 품절률(SKU)`로 바꾸고 SKU 기준 목표 2.5% 별도 수립(굿즈 기준 KPI는 사전집계에 굿즈 기준 품절률이 추가돼야 가능). ③ 목표값 입력칸 `step="1"` → `step="any"`(소수 목표 98.44·2.61·1.0·2.5 공통 적용). ④ 회귀 테스트 `test_kpi_inventory.mjs` 신설 — 실제 `computeKPIs()`를 `dashboard_period_summary.csv`에 물려 목표축 혼선 재발을 검출. ⑤ **장기미회전 재고 비율** 신규 — 졸업 검토 탭의 `psrStagnantList_()`를 재사용(정의 이중화 방지)하고 분모도 같은 모집단(관리대상 Product Parts)에서 재합산 — 도입 시점 19.9%(2026-08-20 기준, 주차마다 변동). ⑥ **품절 해소 소요일** 신규 — `sales_status_history`의 닫힌 상태 구간에서 최근 12개월 해소분 중앙값(29일, 2026-08-20 기준), `end time` 공란(미해소)·`의도적 품절` 제외. KPI 지표 13 → 16종(도움말 문구는 `12개 지표`로 하드코딩돼 이미 실제와 어긋나 있었음 — 문구·`KPI_DEFS.length` 일치를 테스트로 고정). ⑦ **「미구현 지표」 카드 신규**(`KPI_PENDING`) — 폐기율·예측정확도(WMAPE)·재고 Aging 버킷 3종의 *필요 원천 · 현 상태 · 확보처*를 KPI 트래커 하단에 표로 고정. Aging은 입고 이벤트(발주 원본)·소진(월 판매량 FIFO)이 이미 있어 **2026-01-01 기초재고 레이어 1회분**만 신규(아카이브에 입고수량·파츠코드 컬럼 없음). 구현 시 `KPI_DEFS`와의 중복을 테스트가 검출. ⑧ **GitHub Actions push 경합 수정** — `refresh-all`이 워크플로 4종을 동시 기동하는데 재시도가 없어 늦은 쪽이 `fetch first`로 죽고 산출물이 유실(2026-W37 회의자료 실제 유실). 워크플로 5종 전부 3회 재시도로 교체. 목표값의 `localStorage` 의존은 미해소(별건). |
| **v0.9 (2026.08.25c)** | (로컬→PC 배포) | **사용자 요청 — 발주 현황 드릴다운 모달에 25년 이전 자료 포함(v23.37).** v23.35가 2024년 아카이브를 `D.orderHist`에 실었는데 모달 계열은 `D.orderPrevYear`(전년 한 해)만 보고 있어 2024년 13,145행이 빠져 있었다. 공용 헬퍼 `archiveOrderRows()`/`archiveIssueRows()`/`allOrderRows()`/`allIssueRows()` 신설로 풀을 일원화(`orderHist`가 비었으면 `orderPrevYear` 폴백 — 로드 중 모달을 열어도 종전 범위 유지). 적용: `openProductOrderDetail`·`openGoodsDetail`·`computeBufferRows`. **`computeBufferRows`를 함께 넓힌 이유**: 제품 상세 모달이 여분 수치를 이 함수에서 그대로 가져다 쓰므로(두 화면 일치가 v22.4 설계 의도) 한쪽만 확대하면 모달과 여분 추천 표의 숫자가 어긋난다. 이슈 풀도 같은 이유로 `allIssueRows()`로 맞췄다. **표시**: `yearBreakdownHtml(rows,issueRows,title)` 신규 — 연도별 TASK·발주수량·공급가액·재제작·이슈 + 합계, 아카이브 연도에 배지, 단일 연도면 렌더 생략. 두 모달 최상단에 삽입. **라벨 동적화**: `histRangeLabel()`(로드된 아카이브 연도 → `2024~2026년 전체 이력`)로 하드코딩 `당해+전년` 9곳 교체, `ARCHIVE_LABEL`(옵션 미상 출처)도 `'2025 아카이브'` 고정 → `orderHistYears().join('·')`, 굿즈 모달 토글 칩 «당해+전년»→«전체 이력». `orderHistYears()`는 제품별 판매량 추이(v23.35)에 있던 동일 로직을 끌어올려 재사용. **한계(각주에 명시)**: 2023년 이전은 행 단위 원본이 없어(월 집계 `data_hist.json`뿐) 제품·굿즈 분해 불가라 표에 안 나오고, 이슈는 `issue_2025.csv`만 아카이브에 있어 2024년 이슈 칸이 `—`다. 검증: 로컬 8090에서 `allOrderRows()` 33,687행(2024 13,145 / 2025 13,529 / 2026 7,013), 다연도 제품·굿즈 모달 렌더 후 연도표·추이표 실측 확인. |
| **v0.9 (2026.08.25b)** | (로컬→PC 배포) | **사용자 요청 — 이슈율·발주·매입금액 YoY를 2022~2024까지 확장(v23.36).** 종합 현황에 「연도별 실적 비교 (2022~)」 표(`renderYearlyYoY`) 신규. `buildYearlyBuckets()`가 당해(`D.order`/`D.issue`)+아카이브(`D.orderHist`/`D.issueHist`) 행과 집계(`HIST_AGG`/`HIST_ISSUE`)를 **월(YYYY.M) 버킷**으로 먼저 합친 뒤 연도로 롤업한다 — 월 단위로 만들어야 "1~N월 동기간" 모드와 집계 구간 덮어쓰기가 한자리에서 처리된다. **YoY는 두 해가 모두 기록을 가진 달로 창을 좁혀 계산한다**: 단순 합산 비교는 2022년(6월부터 기록 시작)을 2023년 대비 +134%로 만든다. 좁혀진 YoY에는 `▵`와 사용 구간 툴팁을 붙이고, 합계 칸은 좁히지 않은 전체 값을 유지한다. **이슈 결측 보정**: `issue_2025.csv`가 1~5월 26건 / 7~12월 669건으로 상반기가 비어 있어 그대로 두면 2025 이슈율이 낮게 잡혀 2026이 +2.73%p 악화로 보인다(결측 달 제외 시 −2.31%p 개선). `liveMonths(mon,true)`가 그 해 **최대 월건수의 20% 미만**인 달을 결측으로 보고 YoY 창에서 뺀다(중앙값 기준은 결측 달이 많을수록 함께 내려가 판정이 무뎌졌다 — 2025 1~8월에서 1월 하나만 걸러졌음). 해당 연도에는 `부분기록` 배지와 이슈율 뒤 `*`를 붙이되 합계는 원본 그대로 둔다. 이슈 건수는 2024년까지 task 단위 export·2025년부터 movement 단위라 그 경계(2024↔2025)의 YoY는 산출하지 않는다. 가용성: 매입 2022~, 발주·이슈율 2024~, 이슈건수 2023~. |
| **v0.9 (2026.08.25)** | (로컬→PC 배포) | **사용자 요청 — 2022~2024 협력사별 매입금액·발주 이력 반영(v23.35).** 소스가 두 종류라 산출물도 둘로 나눴다. ① **행 단위(2024)**: `Raw Data_Task (2024)` export 13,145행에 지출결의 CSV에서 조인한 `세금계산서작성월 (from 지출결의)`(task_id 우선·발주번호 폴백, 조인율 91.3%)을 붙여 `CSV_BANK/archive/2024/order_2024.csv` 생성 — 2025 아카이브와 동일 스키마라 기존 코드 경로가 그대로 2024년을 탄다. 미조인 1,148행은 빈 값 유지(2025 아카이브에도 지출결의 미연결 1,408행이 같은 상태 — 과업지시월로 대체 추정하면 기준이 어긋난다). 첨부 URL 컬럼 `과업지시서`(본문 46%·미사용)만 제외해 14.9MB→9.8MB. ② **집계(2022~2023)**: 행 단위 소스 부재(2023 Task export는 이슈 건만 담긴 787행 부분집합, 2022는 export 없음)로 지출결의 4개 연도를 병합·중복 제거(제목+수주처+세금계산서월+금액, 2024↔2025 파일 간 198건 실측 중복) 후 세금계산서작성월로 재버킷해 `data/data_hist.json`(2022.6~2023.12, purchase/purCnt/bySup) 생성. `data_2025.json`과 같은 모양이라 `get2025Month()`가 그대로 읽고 YoY 근사에도 쓰인다. 건수 5건 미만 월(세금계산서월 오기재 3건)은 가짜 저점이 되므로 제외. **대시보드 반영**: `loadPriorYearArchive()`가 작년→과거로 순차 탐색해 없는 해에서 멈추며 전 연도를 `D.orderHist`로 합침(연도 하드코딩·404 노이즈 없음, 작년분은 YoY용 `D.orderPrevYear` 유지, 아카이브는 과업지시연도로 완전 분할되어 교집합 0건 실측). 추이 전용 `getTrendOrders(includeStock)` 신설 — 기간 필터가 걸린 KPI·표는 종전대로 `D.order`만 본다. 매입 추이선(합산·협력사별·업태별) 2022.6~현재 51개월, 굿즈코드별은 집계에 제품 분해가 없어 2024~, 제품별 발주 추세선 2024.1~현재 31개월(차트가 표의 13개월 컷 `shownMonths`를 쓰던 것을 `allMonths`로 교체 — 아카이브를 붙인 뒤 추세가 최근 1년으로 잘렸다), 협력사 상세 연도별 매입추이 2022.6~. 집계 구간은 행 단위 잔여분(세금계산서월이 과거로 찍힌 2024년 소수 행)을 덮어써 집계값을 정답으로 삼는다. **기준 변경(주의)**: 매입 추이선을 재고생산 포함으로 통일 — 지출결의에 재고생산 분리 근거(project)가 없고 2024년 실측 비중 31%라 종전 기준을 유지하면 2023→2024 경계에 정의 차이만의 단차가 남는다. 상단 매입 KPI가 이미 재고 포함 총액을 헤드라인으로 쓰므로 그쪽에 맞췄고, 그 결과 2024~2026 추이값이 종전 대비 약 30% 상승(KPI와 일치 방향). 재현 스크립트 `scripts/build_history.mjs`(`SRC_DIR` 기본 `~/Downloads`). **한계**: 2022~2023은 지출결의(총금액÷1.1) 기준이라 발주 원본 기준보다 3~10% 높음(2025년 대조 실측), 2023 Task 부분집합은 미사용, 이슈 export(2023/2024)는 `이슈카테고리` 부재로 월별 총건수만. |
| **v0.9 (2026.08.18c)** | (로컬→PC 배포) | **사용자 요청 — 협력사 제외 이슈율 시뮬레이션 + 이슈 협력사 귀속 정확도(v23.33).** ① 이슈 현황 상단 `#iss-excl-bar` 신규 — 협력사 칩 클릭 시 `ISS_EXCL`(localStorage `iss_excl_v1`)에 넣고 `renderIssueAnalytics()` 재실행. 제외는 **이슈(분자)·발주 TASK(분모) 양쪽**에 적용(`keepIss`/`keepOrd`)해 "없었다면"이 성립하도록 했고, 제외 전 원본 이슈율(`baseRate`)을 함께 계산해 `5.54% → 4.84% (-0.70%p)` 형태로 병기. 적용 범위는 KPI 3종+차트 3종이며 목록 탭은 원본 유지. ② `issueSupplierOf()`가 `(파트너)발주협력사명 텍스트`를 1순위로 사용 — 종전 `movement_id`→`project_name` 접두 추정은 한 프로젝트에 여러 협력사가 붙을 때 첫 협력사를 반환해 **2026 실측 358건 중 190건(53%) 오귀속**이었다. 이 필드는 원천에서 **15자 절단**되어 오므로(`주식회사 기억니은 (구. 화`) `resolveVendorAlias()`가 수주처 명단과 접두 일치로 복원(후보 1개일 때만, 14종 중 12종 복원·모호 0종, `D.order` 기준 메모이즈). 귀속 성공률 370→381건(99.7%). `issueSupplierOf()`를 공유하는 제품×협력사 매트릭스·협력사 상세 모달·재발 이슈에 함께 반영. |
| **v0.9 (2026.08.18b)** | (로컬→PC 배포) | **사용자 요청 — 굿즈마스터 주간 자동 갱신 등록(v23.32).** `AIRTABLE_SOURCES`에 `goods_master` 항목 추가(`appAbBz1Y48qhpHwz`/`tblbIzywmCfG8OOod`/`viwLATCw4GfnlPr3d`, 졸업 포함 뷰) → 매주 목요일 13:00 `weekly-airtable.yml`에 편입, 덮어쓰기 전 `CSV_BANK/<주차>/goods_master.csv` 자동 아카이브. `scripts/fetch_airtable_sources.mjs`에 소스별 선택 옵션 `fields`(지정 필드만 수신 — 34컬럼 중 5개)·`rename`(원천 필드명→CSV 컬럼명) 추가. **rename이 필요한 이유**: 원천 상태 필드명이 `굿즈 Status_1.goods`라 변환 없이 받으면 새 컬럼으로 뒤에 붙고, 대시보드가 읽는 `굿즈 Status`는 API 미반환 컬럼으로 분류돼 v22.4 수동 컬럼 보존 로직이 옛 값을 매주 이월 — 파일은 갱신되는데 상태만 낡는 조용한 오류. 대시보드도 `goodsStatus(r)` 헬퍼로 두 이름 수용(원천명 우선, `굿즈 Status` 폴백), 파일 서명 `FILE_SIGNATURES`/업로드 `must`에서 상태 컬럼 제외(헤더명이 바뀌면 파일 인식 자체가 실패하던 문제) — 나머지 4개로 판별. |
| **v0.9 (2026.08.18)** | (로컬→PC 배포) | **사용자 제보 — 신규 파츠는 뜨는데 신규 협력사·신규 출시 제품이 공지에 없음(v23.31).** 원인은 공지 3종의 원천 갱신 속도 차이 — 파츠 주간 자동 / 협력사 월간 자동 / `goods_master.csv` 수동(최종 2026-07-15). ① `isScmManagedVendor()`에 **parts.csv 제작협력사 명단** 통과 조건 추가(`partsVendorNames()` 메모이즈) — 발주담당자가 타 파트로 적힌 신규 12곳 중 실제로 파츠를 만드는 9곳(비닐24·혜림봉투·제이디패키지 등)이 살아남음. 비닐24는 8/10 «단가 변동» 공지에는 이미 뜬 상태라 같은 협력사가 한쪽에만 나오는 모순이었음. ② `detectPartsChanges()`에 주간 스냅샷 diff 기반 **신규 제작협력사**(`sup_added`, 비고 "파츠 제작협력사 최초 등록")·**신규 제품**(신규 타입 `product_new`, 🆕 신규 제품(파츠 등록)) 감지 추가 — 실측 협력사 3곳·제품 4건(FFTT·BOOK·SCRD·LTSV). ③ 공용 헬퍼 `splitVendorField()`(«"A", "B"» 복수 협력사 필드를 큰따옴표 경계로만 분리 — `Co., Ltd.` 보호)·`partsVendorSet()`·`partsGoodsNameMap()`·`partsGoodsLaunchMap()` 신규. **오탐 방지**: 굿즈코드/굿즈명 lookup은 졸업 굿즈가 코드 쪽에만 실리는 행이 있어 개수가 어긋남 → 개수 일치 행에서만 짝지음(그대로 쓰면 졸업 굿즈 130여 건 오탐), 재고생산(STCK) 제외. 출시일은 `출시일 (from 1. goods)`(굿즈 마스터보다 최신 — BMLT 07-10→07-21)에서 읽되 미래·미정일 수 있어 **공지 날짜는 파츠 등장 시점**, 출시일은 비고 병기. ④ 출시/신규협력사 박스 표시 조건을 `D.goods_master`/`D.sup` 단독 → `(D.goods_master 또는 D.parts)`/`(D.sup 또는 D.parts)`로 확장, 접힘 요약 칩 건수도 합산. ⑤ 변경 이력 리포트 제품 섹션에 `product_new` 포함. **미해결**: `goods_master.csv`는 여전히 `AIRTABLE_SOURCES` 미등록(수동 업로드) — 파츠 없는 출시/졸업 건은 수동 갱신 때만 반영. |
| **v0.9 (2026.08.12)** | (로컬→PC 배포) | **사용자 요청 — 제품 상세 모달 옵션별 분해(v23.21).** 밸류 메모큐브(점착/비점착×S/M)·데스크테리어 매트(기본형/대형)처럼 옵션이 여러 개인데 산출물이 하나로 합쳐져 옵션별 소진 속도를 못 보던 문제. ① **`itemOption(r)`** 신규 — `구성옵션(인쇄제외)`→없으면 `산출물_옵션`에서 `<산출물핵심명>(<옵션>)` 토큰의 괄호 안만 추출(색상·인쇄방식이 아닌 점착여부·사이즈·용량 등). 한 행에 옵션 2종 이상이면 판정 불가로 비움(6,499행 중 26행). ② **`issueItemOption(r,prod)`** 신규 — 이슈의 `입고물품`은 옵션 없는 대표명이라 `item (통합) (from order)`에서 같은 규칙으로 추출. ③ **`optionSkuMetrics(prod)`** 신규 — `dashboard_sku_snapshot`이 PT번호 단위라 옵션이 이미 분리돼 있음을 이용해 최신 주간 스냅샷에서 현재고·판매 3M/YTD·회전율 YTD·단가를 옵션별로 역매칭. ④ **`goodsOptionCatalog(prod)`** 신규 — `parts.csv`의 `굿즈 연결 현황`으로 굿즈 연결 파츠를 골라 `파츠 유형`으로 분류하고 100/300/500/1000/3000개 구간 표준원가·최대 절감폭 산출. **옵션 판별 기준(사용자 확인 2026-08-12)**: 한 굿즈에 **Product Parts가 2종 이상이면 옵션이 나뉜 것**, **Printing Parts**도 실크/레이저·4도/2도에 따라 옵션이 갈리고 단가·판매가가 다름. Phantom Parts(추상적·조립산출물)·Package Parts(단품박스·OPP)·샘플용 파츠는 제외. ⑤ 모달의 "옵션별 발주량" 카드를 **옵션별 발주·재고 현황**으로 확장(재제작 건수·이슈(품질)·재고·판매·회전율·단가 추가, 회전율 1.5↑ 녹색·0.8↓ 적색) + **옵션 구성·수량별 표준원가** 카드 신규 + 발주/이슈 이력 표에 옵션 컬럼. **메인 표는 불변** — 옵션 분해는 모달 전용 **`detailVariantTag()`** 로만 하고 `variantTag()`는 그대로 뒀다: 그쪽은 `rowGroupKey()`를 통해 발주 현황 메인 표의 제품 행 분리 키라 옵션 표기가 없는 2025 아카이브와 어긋나면 YoY가 끊긴다(v22.17). **회귀 검증**: 수정 전(HEAD)/후 두 벌을 `vm` 샌드박스에 올려 `rowGroupKey`를 발주 6,654행 전부 대조 → **불일치 0건**, 제품 761종 모달 렌더 **예외 0건**. 적용 범위: 옵션별 표 79종·수량별 단가 카드 140종. 한계: 데스크테리어 매트는 재고생산 이력 0건이라 SKU 스냅샷에 없어 재고·회전율 산출 불가(안내 문구 표시), 옵션별 품질이슈 표본이 얇음(3건·2건), 메모큐브 재제작 9건은 취득원가가 전부 0이라 건수만 집계(v23.19 지적과 동일 원인). |
| **v0.9 (2026.08.04b)** | (로컬→PC 배포) | **사용자 요청 — 매출 실측을 연간분석 표 기준에 정렬(VAT 별도 + 출고 마감 컷오프).** "매출 현황(업로드 실측)"과 표 매출액의 1~7월 누계 격차 +10.7%(6.02억)를 **+0.11%(0.06억)** 로 축소. 원인이 날짜 기준이 아니라 세 가지였음을 실측으로 규명: ① **VAT** — `proj_rev.csv`의 `총 매출액`은 출고리스트 '출고처리 금액'과 같은 VAT 포함 금액인데 매입금액·표는 VAT 별도. `getProjRevMap()`에서 `REV_VAT_DIV`(1.1)로 나누고 원 단위 반올림해 매출 소비처 전체(매출 모달·종합현황 KPI·원가 분석 월별/프로젝트별)에 일괄 적용 — **원가율이 분자만 VAT 제외라 과소계산되던 구조적 오류도 함께 교정**(1~7월 재고제외 35.1%→38.6%, 재고포함 53.0%→58.3%). 근거는 `총 매출액`을 `발주 신청일` 월로 묶고 ÷1.1 하면 표의 **수주액과 원 단위까지 일치**한다는 역검증(1월 -965원·2월 +2,191원·4월 -3,491원·6월 -79,136원; 5·7월 부족분은 수주 후 미출고라 proj_rev에 행이 없는 건). ② **귀속월** — 표는 달력월이 아니라 매달 출고리스트 마감 시점까지의 출고분 합계라 달력월 집계로는 월별 ±30% 편차(3월 -32%·4월 +38%·6월 +27%). 귀속월을 `REV_CUTOFF` 상수 + `최종 출고일` 기준으로 산출(`revMonthOfShipDate()`). **확정값: 1월분 ~02-23 · 2월분 ~03-17 · 3월분 ~04-22 · 4월분 ~05-21 · 5월분 ~06-17 · 6월분 ~07-08 · 7월분 ~08-04** — 출고리스트가 그 달 출고분을 **다음 달 중순~하순까지 받아서 마감**하므로 컷오프가 다음 달로 넘어간다. 달력 월말 기준으로 바꾸면 누계 +9.9%·월별 최대 +37.9%로 어긋난다(6/24~6/30 출고분이 표에서는 7월분 리스트에 있음). 미확정 월은 달력월 폴백하되 이미 마감된 달로 되돌리지 않고 다음 달로 넘김(07-08 이후 7월 출고분 → 8월분). 종전 `출고 날짜(월)` 컬럼은 `최종 출고일`의 월과 전수 일치(1,433건 중 불일치 0)했으므로 기준일 자체는 바뀌지 않았고, 복수 출고월 균등분할만 최종 출고월 단일 귀속으로 변경. ③ **집계 모수** — `revenueByMonthAll()`/`updateKPIs()`는 2026 발주 RAW에 프로젝트가 있는 건만 집계한다. 여기서 빠지는 **112건 · VAT 제외 546,875,649원(대부분 1월 출고)** 은 2025년 12월 수주·2026년 1월 출고분으로 **이미 2025년 매출로 마감**된 건이라 2026에 다시 세면 이중계상이 된다(사용자 확인 2026-08-04). *2026 발주 RAW에 없다 = 당해 발주가 없다 = 전년 귀속*이라 이 조건이 그대로 판별자가 된다. 세션 중 한때 "표는 출고 시점 기준이니 포함이 맞다"고 보고 필터를 제거했으나 위 회계 사실 확인으로 원복. **최종 표 대비 월별 오차 -0.4/+2.2/+3.1/+0.0/-1.1/+1.6/-1.7% · 누계 +0.67%**(전 구간 ±3.2% 이내 · |오차|합 10.2%p, 4월은 318,791원 차이). 정확히 0.0%는 날짜 단위로 불가능 — 하루치 출고 중앙값이 3,455만원이라 경계 한 칸에 월 금액이 3~5% 점프하고, 전 구간 DP 최적화 시 월별 최악의 하한이 2.17%(그 경우 누계 -0.97%). 5·6·7월 경계는 1~4월 고정 하 전수 탐색 최적값이며, 6월분을 07-07로 당기면 6월·7월이 동시에 나빠진다(07-08에 6월 몫이 몰려 있음). ④ **프로젝트 귀속 원가율**(`projectCostByMonth()`, v23.14 후속): 모달의 매입 2행은 세금계산서작성월 기준(=월별 지출 파악용)이라 매출과 달이 어긋나 월별 원가율이 24~54%로 튀었다 — 각 발주 프로젝트의 원가를 그 프로젝트의 **매출 귀속월**에 얹는 행 2개와 KPI 카드 1장을 추가(매입 마감이 3월이어도 매출이 4월이면 4월 원가로 본다). `renderCostMonthly`와 같은 귀속 규칙이며 세금계산서월 기준 행은 목적이 달라 그대로 병기. **목표선 45% 미만**(`COST_RATE_TARGET`, 45%↑ 빨강·40~45% 주황 — 매출 100 = 외주생산제조비 45 + 물류비·판관비 15 + 마진 40). ⑤ **선매입 재고 투입원가**(`stockInputCost()`): 재고 출고분은 재고생산 발주가 프로젝트에 안 붙어 공급가액 0으로 잡혀 원가가 통째로 누락됐다(원가 0원 프로젝트 61건·매출 1.51억). 재고생산 원가 전체를 얹으면 과대계상이라 **실제 투입 수량 × 파츠 표준원가**로만 배분 — `출고자재 (from movement_투입이동)`의 PT번호와 `출고자재_수량`을 콤마 분해해 인덱스로 짝짓고 `parts_master.표준원가`(329종, `buildPartsStdCost()` 캐시)를 곱한다. 출고 수량에 여분이 이미 반영돼 있어(고객주문 100→출고 103) 1.03 계수는 불필요. ⚠ `출고자재_수량`은 `"55, 55"` 형태 다중값이라 `pa()`로 읽으면 5555가 되므로 전용 파서 필수. 2026년 1,386행·1,051,334,383원이 새로 잡혀 1~7월 원가율 36.4%→**53.2%**(목표 45% 대비 +8.2%p 초과, 2026-08-05 데이터 기준 — 주간 CSV 갱신에 따라 변동). 물류비·공통고정비는 여전히 제외라 실제 총원가는 더 크고, 표준원가 미보유 파츠는 배분에서 빠져 다소 과소. ⑥ **마진 분포도 2종**(`marginDistHTML()`·`renderMarginDistChart()`, 원가 분석 → 프로젝트별 상세): 제품(산출물) 단위 매출 실측이 없어 — 행 단위 판매가 필드 3종이 전 행 공란이고 `본품 매출 합계액`은 프로젝트 실측과 3배 어긋남 — 두 축으로 우회한다. ⓐ 프로젝트 원가율 분포 히스토그램(실측 매출 1,384개 전량, 중앙값 56.2%, 목표 45% 초과 1,040개=75%) ⓑ 굿즈코드별 마진 Top20(**단일 굿즈코드 프로젝트 878개**만 — 매출 23.7억·실측 매출의 41%; 여러 제품이 섞인 프로젝트는 매출을 쪼갤 근거가 없고 원가 비율 배분 시 전 제품 마진이 같아져 무의미). 기존 '프로젝트 원가율 순위(Top 30)'의 `taskCost`도 `공급가액 + stockInputCost()`로 통일. 이 컷오프 패턴은 112건을 뺀 뒤에야 드러났다 — 5.47억이 1월에 섞여 있는 동안에는 컷오프가 월말 근처로 잘못 잡혔다. 화면에 `cutoffRow()`(마감 출고일)·`diffRow()`(실측−표, 합계는 양쪽 마감월만) 2행 신규 + 카드 비고·모달 안내문 갱신. **운영 주의: 마감일은 매달 달라 고정 규칙이 없으므로 새 달 확정 시 `REV_CUTOFF`에 날짜 추가 필요.** |
| **v0.9 (2026.08.04)** | (로컬→PC 배포) | **사용자 요청 — 빅딜 판정 기준 통일(제품별 표준분포 80% 범위 초과).** 화면마다 빅딜 기준이 달라 같은 발주 건의 판정이 엇갈리던 문제 해결 — 발주 현황 제품 상세 드릴다운은 `건당 평균 발주수량×1.1`(v22.17), 제품별 판매량 추이는 IQR 울타리 `Q3+1.5×IQR`(v23.7)을 각각 쓰고 있었음. 공통 함수 **`bigDealStat(qtys)`**(전역 유틸, `partsType()` 아래) 신설: 그 제품의 발주수량 분포에서 μ·σ(모표준편차)를 구해 정규분포 중앙 80% 구간 상단인 **`cut = μ + 1.2816σ`**(`BIGDEAL_Z`)를 반환하고, 표본 5건 미만이거나 σ=0이면 `cut=Infinity`로 판정을 보류. 표시용 정수 경계는 `bigDealMinQty(cut)=⌊cut⌋+1`로 "cut 초과" 판정과 경계를 정확히 일치시킴. 두 호출부 모두 `qty > cut` 단일 규칙으로 교체 — ① `openProductOrderDetail()`: `bigDealQty`/`bigDealOrders`/행 배지 산출을 교체하고 기준 수량 옆에 평균·표준편차·표본 수를 병기(보류 시 사유 표기) ② `renderProductTrend()`: 지역 `percentileOf()`+IQR 블록을 제거하고 `bigDealStat()` 호출로 대체, 제품별 `bigDealMin`을 배지 툴팁에 노출. 요약 카드 설명 문구가 v23.7 IQR 전환 때 갱신되지 않아 옛 기준("한 건이 그 제품 전체의 80% 이상")을 표시하던 것도 함께 정정. **선택 근거**: 실측 `CSV/order.csv`(3,139건·284제품)로 IQR(254건 8.1%)·실측 P80(555건 17.7%)·μ+1.2816σ(310건 9.9%)·μ+0.8416σ(408건 13.0%)·평균×1.1(760건 24.2%)을 비교해 사용자가 μ+1.2816σ 채택. σ가 극단값에 오염되는 성질상 UV펜(최대 16,040개)처럼 초대형 1건이 있는 제품은 기준이 2,320개로 올라가 제외 건수가 IQR보다 줄어드는 특성이 있음(트레이드오프 인지 후 확정). |
| **v0.9 (2026.07.31b)** | (로컬→PC 배포) | **사용자 요청 — 매출 현황 목표 대비 모달 신규.** 종합 현황 "매출 현황" KPI 카드의 클릭 동작을 원가 분석 화면 이동 → 상세 모달(`openModal('revenue')`/`renderRevenueModal()`)로 교체(원가 분석 이동은 모달 내 링크로 유지). ① KPI 3장 — 선택 기간 목표 수주액 / 매출 현황(업로드 실측) + **목표 대비 달성률** / 매입금액(재고제외) + **매출 대비 비중**(50%↑ 주황·70%↑ 빨강, 카드 비고·원가분석 원가율과 동일 정의). ② **2025 vs 2026 선형 추이 차트**(`ch-rev-plan`) — 2026 수주액·2026 매출액·2025 수주액(점선)·목표(연회색 점선) 4계열, `interaction:{mode:'index'}`로 같은 월 전 계열 툴팁, 미발생 월은 `null`로 치환해 선이 0으로 떨어지지 않게 함. ③ 월별 표(1~12월+합계) 9행 — 선택 기간 월은 파란 배경 강조(월/분기/기간지정 모두 대응, `planSelMonthIdx()`), 선택 기간이 대상 연도 밖이면 안내 배너 후 연간 기준 렌더. **데이터 출처가 이원적**: 목표·수주액·전년·표매출액은 구매조달팀 연간분석 01-A/01-B 표를 옮긴 `SALES_PLAN` 상수(CSV 자동로드 대상 아님 — 표 갱신 시 배열 교체 필요)이고, "매출 현황"·"매입금액" 행은 기존 `getProjRevMap()`/`purchaseByMonth()` 실측 경로를 월별로 재집계(`revenueByMonthAll()`/`purchaseByMonthAll()`)한 값이라 산출 기준이 달라 표의 "2026년 매출액"과 차이가 날 수 있음(모달에 주석 명시). 전사 검증: 산출된 월별 목표대비·YoY %가 원본 표와 전부 일치. `curOrder` 12개월 합이 5,891,700,000으로 표 합계 5,891,690,000과 1만원 차이가 있으나 **사용자 판단으로 무시 확정**(화면 합계는 배열 총합으로 표시되어 내부 일관성은 유지). **(같은 날 보강)** 매입 비중을 **재고제외/재고포함 두 기준으로 병기** — KPI 카드 3번을 "매입 / 매출 비중 (재고제외)"를 메인 값으로 재구성하고 비고에 재고포함 비중·양쪽 매입금액을 표시, 월별 표에 `매입금액 (재고포함)`·`매입 / 매출 비중 (재고포함)` 2행 추가. 비중 행은 `ratioRow(label,num,den,idxs,colFn)` 헬퍼로 공통화(합계 열은 배열 총합끼리 나눈 가중 비중이지 월 평균이 아님). 실측 2026: 재고제외 연간 36%·7월 31.1% / 재고포함 연간 55%·7월 50.8%. |
| **v0.9 (2026.07.31)** | (로컬→PC 배포) | **사용자 요청 — 연간 이벤트 캘린더 신규 + NEST 바로가기.** ① 사이드바 "운영 서비스" 최하단에 **KPI종합분석-NEST ↗** 외부 링크 추가(`.sb-item.ext`, `nest.sincerely.work/workspace/kpis/combined`). ② 사이드바 **"일정관리" 그룹 신설 + 연간 이벤트 캘린더 화면(`sec-calendar`/`renderCalendar()`) 신규** — 구매조달팀 「연간 주요 이벤트 마스터」를 옮긴 화면으로, 원천이 GitHub 마크다운 문서라 CSV 자동로드 파이프라인을 타지 않고 코드 내 상수(`CAL_MASTER` 9개 업무 · `CAL_WEEKLY` 주간 정기회의 2건 · `CAL_CAT_COLOR` 7개 구분)로 관리한다. 구성은 (a) 주간 정기회의 칩(금 11:00 구매조달 파트장 회의 · 목 15:00 외주생산파트 회의) (b) 9개 업무 × 12개월 간트 타임라인(`calGanttHtml()`, 구분별 색상 · 막대 title에 착수~종료) (c) 월 타일 12개(업무 건수 · 특이사항 ⚠ 배지 · 당월 강조) 클릭 시 해당 월 상세 표(`calSelectMonth()`, 진입 시 당월 자동 선택) (d) **공급망 특이사항 · 수시 이슈** — 협력사 휴가·발주 마감처럼 연간 마스터에 없는 비정기 이슈를 직접 추가/삭제(`calAddNote()`/`calDelNote()`), `localStorage.cal_special_notes_v1` 저장이라 브라우저 단위로 유지되며 월 타일 배지·월별 상세 강조 행에 함께 반영. 업무별 월 표시 범위는 마스터 §3 월별 뷰와 1:1로 검증(명절연휴 리드타임 1·2·7·8·9·12월, 춘절 매입 11·12·1·2월 등 사전 준비 기간 포함 — §2 착수/종료만 보면 누락되는 지점). 사용자 입력은 전부 `esc()` 경유 출력. ③ **(같은 날 보강)** `d-holi` 담당에 구매전략파트 추가, 특이사항에 `dept` 필드·등록 폼 담당 드롭다운 신규(기존엔 월별 상세에서 담당이 항상 `—`), 비벨라 이탈리아 공장 휴가를 **8월 → 7월 표시**로 조정(실제 휴가는 8월이지만 구매전략파트 사전 대응 시점이 7월 — 기간 문구에 양쪽 병기). 기본 항목 정의가 바뀌어도 이미 저장한 브라우저에 반영되지 않으므로 키를 `cal_special_notes_v1` → `v2`로 올리고, `calLoadNotes()`가 기본 항목(`seed-*`)만 최신 정의로 교체하고 사용자 추가분(`note-*`)은 v1에서 이월하도록 이관 로직 추가. |
| **v23.5** | (로컬→PC 배포) | **사용자 제보 — 원가 분석 프로젝트별 상세 검색 안 됨.** `renderCostByProject()`의 프로젝트 선택 `<select>`(1,300여 개, 매출액 내림차순)가 상단 전역 검색창과 무관해 프로젝트명·번호로 찾을 수 없었음 → `<input list="cost-proj-datalist">`(네이티브 datalist)로 교체(같은 `id="cost-proj-sel"` 유지해 Top 30 표 행 클릭 코드 그대로 동작), `onCostProjSearch()` 신규 — 정확 일치 우선, 없으면 대소문자 무시 부분 일치 첫 건을 선택해 재렌더. 검색 대상 목록은 전역 `COST_PROJ_LIST`(`renderCostByProject()`가 채움). 프로젝트명·번호 양쪽 실데이터 검증. |
...
**"지금 새로고침" 버튼 (v20 신규, v23.9 전면 수정)** — 매주 목요일을 기다리지 않고 필요할 때 바로 원천 데이터를 재수집하고 싶을 때 쓴다. 업로드 바의 🔄 지금 새로고침 클릭 → `refreshAirtableNow()`가 `POST /repos/{owner}/{repo}/dispatches`로 **`repository_dispatch` 이벤트 `refresh-all`** 을 보내고, 이 이벤트를 구독하는 **네 워크플로**(`weekly-airtable.yml`·`weekly-google-sheets.yml`·`weekly-meeting-agenda.yml`·`monthly-suppliers.yml`)가 동시에 돈다. **v23.38: 넷이 동시에 `git push origin main`을 하므로 늦게 도착한 쪽이 `! [rejected] (fetch first)`로 죽는다** — 재시도가 없어 2026-W37 회의자료 JSON이 실제로 유실됐다. 다섯 워크플로 전부 `pull --rebase` + `push`를 3회 재시도로 감쌌고, 재시도를 다 쓰면 스텝이 `exit 1`로 죽는다(GitHub 기본 셸이 `bash -e`라 `sleep`이 마지막 명령이면 실패가 삼켜진다). 공유 `concurrency` 그룹은 쓰지 않았다 — 대기열이 1개뿐이라 `refresh-all` 한 번에 워크플로가 통째로 취소된다. `pollAirtableRefresh()`가 20초 간격(최대 10분)으로 세 워크플로의 완료를 함께 폴링하다가, 하나라도 성공하면 GitHub Pages 재배포를 60초 기다린 뒤 `autoLoadRaw()`로 최신 CSV를 화면에 반영한다. 발주 진행현황(`progress_*`)·매출결산(`proj_rev`)도 이 경로에 포함된다.
...
**과거 연도 소급 반영 (v23.35, 2026-08-25)** — 위 워크플로는 "올해가 끝나면 한 해 보존"이라 그 이전 연도는 자동으로 채워지지 않는다. 2022~2024는 Airtable 수동 export로 소급했고, 재현 스크립트가 `scripts/build_history.mjs`다(`SRC_DIR` 기본 `~/Downloads`). 두 가지 산출물이 나온다: ① Task export가 완전한 연도는 지출결의 CSV에서 `세금계산서작성월 (from 지출결의)`을 조인해 `CSV_BANK/archive/{연도}/order_{연도}.csv`로 — 2025 아카이브와 스키마가 같으므로 `loadPriorYearArchive()`가 자동으로 집어간다(작년→과거로 순차 탐색해 order 파일이 없는 해에서 멈춘다, 즉 **아카이브 연도는 연속이어야 한다**). ② Task export가 없거나 부분집합인 연도(2022, 2023)는 지출결의 CSV 집계만 `data/data_hist.json`으로 — 협력사·합산 매입금액까지만 조회되고 제품·굿즈코드 분해는 불가능하다. 추가 연도를 소급할 때는 이 스크립트의 연도 목록과 `DROP` 컬럼만 손보면 된다. **화면 쪽 소비 규칙(v23.37)**: 기간 필터가 걸린 KPI·표는 `D.order`만, 추이 차트는 `getTrendOrders()`, **드릴다운 모달·여분 산식은 `allOrderRows()`(당해+아카이브 전 연도)** 를 쓴다 — 새 화면을 붙일 때 어느 풀을 볼지부터 정하고, 라벨은 `histRangeLabel()`로 뽑아 연도를 하드코딩하지 않는다.
