...
> 버전: v12 | 갱신일: 2026-07-10 | 작성: 이난영 / Claude AI  
...
| 버전 스냅샷 | `scm_dashboard_v12.html` (index.html 복사본, 이전 버전은 `archive/`) |
...
└──────────────┬───────────────────────────────────────────────────┘
               ...
               ▼
┌──────────────────────────────────────────────────────────────────┐
│         접속자 브라우저 (index.html · v12 · 약 5,062줄)              │
│                                                                    │
│  ┌───────────────────────┐  ┌────────────────────────────────┐   │
...
```
SCMDASHBOARD/
├── index.html                       ← 운영 원본 (GitHub Pages 직접 서빙 · v12 · 약 5,062줄)
├── scm_dashboard_v12.html           ← 현행 버전 스냅샷 (index.html 복사본)
├── archive/                         ← 이전 버전 스냅샷 (v3~v11)
├── CSV/                             ← 자동 로드 대상 CSV
...
├── docs/
│   ├── SCM_DASHBOARD_ARCHITECTURE.md       ← 이 파일 (v12 갱신)
│   ├── purchase_dashboard_migration_strategy.md  ← 구매파트 Apps Script 이식 전략 (v10 반영 현황 기준, v11/v12는 범위 밖)
│   ├── SCM_DASHBOARD_로직설명_v12.html  ← 로직 설명서 (v12 갱신)
│   ├── SCM_DASHBOARD_사용자가이드_v12.html  ← 사용자 가이드 (v12 갱신)
│   ├── SCM_KPI_리포트_2026Q2.xlsx
│   └── archive/                     ← 구버전 (v10 이하 로직설명/사용자가이드, V4_로직설명_v7.html 등)
├── deploy_v12.ps1                   ← 배포 스크립트 (git 자가복구 + 버전 정리 + 안전 동기화 + 문서/구파일 정리)
├── .nojekyll                        ← Pages Jekyll 비활성화 (빌드 실패 방지)
└── .claude/                         ← Claude 설정
```
> `_merge_staging/`(v10 3-way 병합 1회성 산출물), `deploy_v10.ps1`, `merge_resolve_v10.ps1`은 v12 배포 시 정리(archive 이동 또는 삭제) 대상 — §9 참고.

### 버전 규칙 (v7~)
- **운영 원본은 `index.html`** — 수정 작업은 index.html에 직접, 로컬 확인은 수동 업로드 모드
- **배포는 `deploy_v{N}.ps1`(현재 `deploy_v12.ps1`)** — 실행 시 자동으로:
...
| 3 | `deploy_v12.ps1` 실행 | 고정명 복사 + 커밋 + 푸시 자동. 또는 GitHub 웹에서 고정명 파일 직접 덮어쓰기 |
...
| 2 | `CSV/`에 저장 후 `deploy_v12.ps1` 실행 | 외주생산파트와 동일 배포 경로 공유 |
...
*문서 기준: index.html (scm_dashboard_v12.html) v12 (2026-07-10) · 약 5,062줄*  
