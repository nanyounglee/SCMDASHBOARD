...
// 이슈발생협력사: 품질등급 C·D 또는 재제작(실패비용) 발생 협력사만 표기
//   (중요도 중~상 heuristic — 별도 중요도 필드 생기면 교체). 그 외 협력사는 제외.
// 품절: catSupOf(partsNm) — PT코드로 parts.csv(굿즈 카테고리·제작협력사) 우선
//   조회 → 없으면 parts_master(pmLookup) 폴백 → 수주처는 supFromParts(D.order 역조회)
//   최종폴백. 카테고리가 "Drinkware, 기타"처럼 다중값이면 '기타'만 걸러내고 나머지를
//   join(콤마 목록에서도 기타 제외 — 5-1~5-5 그루핑과 카테고리 표기 통일). 표시 품명은
//   stripPt()로 "PT****-" 접두어 제거.
//
