...
### 4-33. 외주생산파트 주간리포트 생성 — genWeeklyOutsource() (v22.16 · v23.8 · 2026.08.05 개편)
...
//
// ─ 섹션 5-0: 이슈발생협력사 + 협력사 품절 (v23.8 개편) ────────
// 이슈발생협력사: 품질등급 C·D 또는 재제작(실패비용) 발생 협력사 중
//   **repeatSups(2주 연속)에 해당하는 협력사만** 표기(중요도 중~상 heuristic —
...
//   "PT****-" 접두어 제거.
//
// ─ 섹션 5-1: 카테고리별 요약 (v23.8 표→줄글, 2026.08.05 번호 5-1~5-5→5-1) ─
// parts_master 굿즈카테고리로 이슈·단가·품절을 그루핑 → 카테고리별 "제품명 +
//   이벤트(단가 인상/인하·유형 이슈·품절)" 줄글 bullet. 단가 방향은 변동사항
//   텍스트에서 인상/인하 판별. 품명은 trim 후 중복 제거. '기타' 카테고리 제외.
//   상세(수주처·내용·SCM대응)는 섹션 2·4-1·5-0 참조(중복 제거가 목적).
//
...
//
// ─ 섹션 7: 운영 전략 제안 QCD (v23.8 표→줄글) ────────────────
// Q(품질): 3주/2주 연속 이슈 협력사, 실패비용 집중(YTD 매입 0.3%↑),
//   품질등급 불일치를 크리티컬 중심 한 줄 bullet로.
// C(원가): 카테고리별 원가변동을 1줄로 통합(총건수 + 최다 카테고리 비중), '기타' 제외.
//   + 복합 리스크(단가변동+이슈 동시 발생) 협력사 표기.
// D(납기): 미입하율·긴급률 지표 + 해당 협력사 줄글. 긴급 협력사는 건수 상위
//   3곳만 표기(2026.08.05: 5곳→3곳, 사용자 요청).
```
