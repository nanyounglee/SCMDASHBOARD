...
## v22~v23 반영 현황 (2026-07-24 갱신)

v10 이후 약 2주간 배포된 v22.1~v23에서 아래 항목이 추가로 해소됐다. 이 절이 이 문서의 최신 기준이며, 위 "v10 반영 현황" 표는 과거 스냅샷으로 남겨둔다.

| 갭 (v10 시점 진단) | v22.x~v23 상태 |
|---|---|
| 입고예정금액 없음 | **해소(v22.1)**. `raw_발주.csv` 미입하 발주 × `parts_master.csv` 표준원가로 계산, 카드 클릭 시 SKU별 상세 |
| 회전율 분모 차이(1M/3M/YTD 평균재고 vs 현재 재고금액) | **여전히 미검증**. Agg 경로는 CSV의 `turnover_1m/3m/ytd`를 그대로 표시하며, GSheets가 실제로 기존 Apps Script와 동일 산식으로 계산하는지는 아직 대조되지 않았다 |
| Product Parts 필터 약함(`관리대상여부` 공란 시 전체 포함) | Agg 경로(`isAggManaged`)는 `is_managed==='Y'`만 인정해 레거시보다 엄격 — 유지 |
| 매입검토/시즌계획 저장 기능 | **의도적으로 계속 미해소**. 화면·업로드 슬롯은 유지하되 저장은 `localStorage` 임시값뿐 — 운영 원본은 계속 구글시트 |
| 협력사재고 품절 업로드 기능 없음 | **부분 해소**. `stockout_list.csv`의 `재고소유구분 (from sync_parts)` 실측 필드로 "협력사 소유 재고 품절"은 공급망 포트폴리오의 방문 추천 협력사(v23)에서 판정 가능해졌으나, 협력사가 직접 제출하는 전용 수집 채널은 여전히 없음 |
| 안전재고 산식(EOQ) | 재작업 완료(v22.7~v22.8) — 일수 근사 → 목표서비스수준 Z-score 통계식으로 교체, 리드타임도 파츠별 실측치 우선 사용. 단, 발주비용·보관비율 파라미터는 여전히 업계 통상 가정값(사내 실측 아님) |
| 재고 소유구분 3분류(직접제조/협력사 소유/협력사 상시재고 활용) | **미해소**. 차기 프로젝트(WS2)로 이월 — 지금은 재고소유구분 실측값이 있는 화면(품절)에서만 부분적으로 활용 |
| `inv_weekly.csv`/`sales_monthly.csv` 레거시 경로 | v22.8에서 완전 폐기 — 두 CSV 모두 2026-07-07 이후 갱신이 멈춰 있었고, 사전집계 3종 + `dashboard_sku_monthly.csv`(EOQ 수요표준편차·SKU 판매추이 소스)가 전량 대체 |
