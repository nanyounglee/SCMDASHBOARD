...
for label, val in [("인쇄 최소금액", 36000), ("코팅 최소금액", 45000), ("톰슨 최소금액", 50000),
                   ...
                   ("합지 최소금액", 80000), ("관리비 최소금액", 120000),
                   # 최소마진은 v1.0(외주 2026-09-11판)부터 수량 구간제다 — 세 값을 다 노출한다
                   ("마진 최소금액(500개 이상)", 200000), ("마진 최소금액(100~499개)", 100000),
                   ("마진 최소금액(100개 미만)", 50000),
...
# 최소마진은 수량 구간제 — 500개↑ 20만 / 100~499개 10만 / 100개 미만 5만
summary("적용 최소마진", f"=IF({QTY}>=500,{mn['마진 최소금액(500개 이상)']},"
                       f"IF({QTY}>=100,{mn['마진 최소금액(100~499개)']},{mn['마진 최소금액(100개 미만)']}))")
summary("마진액", f"=MAX({S['총원가']}*{S['적용 마진율']},{S['적용 최소마진']})")
